--  SqlServer.2008

IF (OBJECT_ID(N'[Issue4671Entity1]', N'U') IS NOT NULL)
	DROP TABLE [Issue4671Entity1]



--  SqlServer.2008

IF (OBJECT_ID(N'[Issue4671Entity1]', N'U') IS NULL)
	CREATE TABLE [Issue4671Entity1]
	(
		[Id]    int  NOT NULL IDENTITY,
		[Value] int  NOT NULL,

		CONSTRAINT [PK_Issue4671Entity1] PRIMARY KEY CLUSTERED ([Id])
	)



--  SqlServer.2008

CREATE TABLE [tempdb]..[#Issue4671Entity1TMP]
(
	[Id]    int  NOT NULL IDENTITY,
	[Value] int  NOT NULL,

	PRIMARY KEY CLUSTERED ([Id])
)



--  SqlServer.2008

INSERT INTO [Issue4671Entity1]
(
	[Value]
)
VALUES
(
	1
)



--  SqlServer.2008

INSERT INTO [tempdb]..[#Issue4671Entity1TMP]
(
	[Value]
)
VALUES
(
	2
)



--  SqlServer.2008

SELECT TOP (2)
	[t1].[Id],
	[t1].[Value]
FROM
	[Issue4671Entity1] [t1]



--  SqlServer.2008

SELECT TOP (2)
	[t1].[Id],
	[t1].[Value]
FROM
	[tempdb]..[#Issue4671Entity1TMP] [t1]



--  SqlServer.2008

IF (OBJECT_ID(N'[tempdb]..[#Issue4671Entity1TMP]', N'U') IS NOT NULL)
	DROP TABLE [tempdb]..[#Issue4671Entity1TMP]



--  SqlServer.2008

IF (OBJECT_ID(N'[Issue4671Entity1]', N'U') IS NOT NULL)
	DROP TABLE [Issue4671Entity1]



