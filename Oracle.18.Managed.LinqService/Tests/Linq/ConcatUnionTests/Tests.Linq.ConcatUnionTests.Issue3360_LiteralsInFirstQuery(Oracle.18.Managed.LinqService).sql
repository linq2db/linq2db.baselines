...
FROM
	(
		SELECT
			r."Id",
			CAST(5 AS Number(3)) as "c1",
			CAST(5 AS Number(3)) as "c2",
			CAST(:Guid AS Raw(16)) as "c3",
			CAST(:GuidN AS Raw(16)) as "c4",
			CAST('ENUM1_VALUE' AS VarChar(11)) as "c5",
			CAST('ENUM2_VALUE' AS VarChar(11)) as "c6",
			CAST(1 AS Char(1)) as "c7",
			CAST(0 AS Char(1)) as "c8"
		FROM
			"Issue3360Table1" r
		WHERE
			r."Id" = 2
