-- DB2 DB2.LUW DB2LUW

WITH CTE_1
(
	"Id",
	"field_1",
	"Name",
	"DeptCount"
)
AS
(
	SELECT
		"c_1"."Id",
		ROW_NUMBER() OVER (ORDER BY "c_1"."Id"),
		"c_1"."Name",
		(
			SELECT
				COUNT(*)
			FROM
				"Department" "d"
			WHERE
				"d"."CompanyId" = "c_1"."Id"
		)
	FROM
		"Company" "c_1"
	ORDER BY
		"c_1"."Id"
),
CTE_2
(
	"Detail_Id",
	"Detail_Name",
	"Detail_EmpCount",
	"Key_1",
	"field_1"
)
AS
(
	SELECT
		"d_1"."Id",
		"d_1"."Name",
		(
			SELECT
				COUNT(*)
			FROM
				"Employee" "e"
			WHERE
				"e"."DepartmentId" = "d_1"."Id"
		),
		"kd"."v0Id",
		ROW_NUMBER() OVER (ORDER BY "d_1"."Id")
	FROM
		(
			SELECT DISTINCT
				"t1"."Id" as "v0Id"
			FROM
				CTE_1 "t1"
		) "kd"
			INNER JOIN "Department" "d_1" ON "d_1"."CompanyId" = "kd"."v0Id"
	ORDER BY
		"d_1"."Id"
),
CTE_3
(
	"Detail_Id",
	"Detail_DepartmentId",
	"Detail_Name",
	"Detail_Salary",
	"field_1",
	"Key_1"
)
AS
(
	SELECT
		"nd"."Id",
		"nd"."DepartmentId",
		"nd"."Name",
		"nd"."Salary",
		ROW_NUMBER() OVER (ORDER BY "nd"."Id"),
		"pd"."v0Detail_Id"
	FROM
		(
			SELECT DISTINCT
				"t2"."Detail_Id" as "v0Detail_Id"
			FROM
				CTE_2 "t2"
		) "pd"
			INNER JOIN "Employee" "nd" ON "nd"."DepartmentId" = "pd"."v0Detail_Id"
	ORDER BY
		"nd"."Id"
)
SELECT
	"t6"."Item1",
	"t6"."Item2",
	"t6"."Item3",
	"t6"."Item4",
	"t6"."Item5",
	"t6"."Item6",
	"t6"."Item7",
	"t6"."Item1_1",
	"t6"."Item2_1",
	"t6"."Item3_1"
FROM
	(
		SELECT
			CAST(0 AS Int) as "Item1",
			"t3"."Key_1" as "Item2",
			"t3"."field_1" as "Item3",
			"t3"."Detail_Id" as "Item4",
			"t3"."Detail_Name" as "Item5",
			"t3"."Detail_EmpCount" as "Item6",
			CAST(NULL AS Int) as "Item7",
			CAST(NULL AS Int) as "Item1_1",
			CAST(NULL AS NVarChar(255)) as "Item2_1",
			CAST(NULL AS Int) as "Item3_1"
		FROM
			CTE_2 "t3"
		UNION ALL
		SELECT
			CAST(1 AS Int) as "Item1",
			"t4"."Key_1" as "Item2",
			"t4"."field_1" as "Item3",
			"t4"."Detail_Id" as "Item4",
			"t4"."Detail_Name" as "Item5",
			"t4"."Detail_DepartmentId" as "Item6",
			"t4"."Detail_Salary" as "Item7",
			CAST(NULL AS Int) as "Item1_1",
			CAST(NULL AS NVarChar(255)) as "Item2_1",
			CAST(NULL AS Int) as "Item3_1"
		FROM
			CTE_3 "t4"
		UNION ALL
		SELECT
			"t5"."Item1",
			"t5"."v0Id" as "Item2",
			"t5"."v1field_1" as "Item3",
			CAST(NULL AS Int) as "Item4",
			CAST(NULL AS NVarChar(255)) as "Item5",
			CAST(NULL AS Int) as "Item6",
			CAST(NULL AS Int) as "Item7",
			"t5"."v0Id" as "Item1_1",
			"t5"."v2Name" as "Item2_1",
			"t5"."v3DeptCount" as "Item3_1"
		FROM
			(
				SELECT
					CAST(2 AS Int) as "Item1",
					"p"."Id" as "v0Id",
					"p"."field_1" as "v1field_1",
					"p"."Name" as "v2Name",
					"p"."DeptCount" as "v3DeptCount"
				FROM
					CTE_1 "p"
			) "t5"
	) "t6"
ORDER BY
	"t6"."Item1",
	"t6"."Item2",
	"t6"."Item3"

