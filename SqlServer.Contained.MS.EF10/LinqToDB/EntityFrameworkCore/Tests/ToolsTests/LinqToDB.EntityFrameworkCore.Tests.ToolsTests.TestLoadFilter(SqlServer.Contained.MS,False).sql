Parameters:
@ef_filter__p9='?' (DbType = Boolean), @ef_filter__p1='?' (DbType = Boolean), @ef_filter__p5='?' (DbType = Boolean), @ef_filter__p11='?' (DbType = Boolean), @ef_filter__p7='?' (DbType = Boolean)
...
LEFT JOIN (
    ...
    INNER JOIN (
        ...
        FROM [Orders] AS [o0]
        WHERE @ef_filter__p9 = CAST(1 AS bit) OR [o0].[IsDeleted] = CAST(0 AS bit) OR [o0].[IsDeleted] = CAST(0 AS bit)
    ) AS [o1] ON [o].[OrderID] = [o1].[OrderID]
    INNER JOIN (
        SELECT [p0].[ProductID], [p0].[SupplierID]
        FROM [Products] AS [p0]
        WHERE (@ef_filter__p1 = CAST(1 AS bit) OR [p0].[ProductID] > 2) AND (@ef_filter__p1 = CAST(1 AS bit) OR [p0].[Discontinued] = CAST(0 AS bit)) AND (@ef_filter__p5 = CAST(1 AS bit) OR [p0].[IsDeleted] = CAST(0 AS bit) OR [p0].[IsDeleted] = CAST(0 AS bit))
    ) AS [p1] ON [o].[ProductID] = [p1].[ProductID]
    LEFT JOIN (
        SELECT [s].[SupplierID]
        FROM [Suppliers] AS [s]
        WHERE @ef_filter__p11 = CAST(1 AS bit) OR [s].[IsDeleted] = CAST(0 AS bit) OR [s].[IsDeleted] = CAST(0 AS bit)
    ...
    LEFT JOIN (
        ...
        FROM [Products] AS [p2]
        WHERE (@ef_filter__p1 = CAST(1 AS bit) OR [p2].[ProductID] > 2) AND (@ef_filter__p1 = CAST(1 AS bit) OR [p2].[Discontinued] = CAST(0 AS bit)) AND (@ef_filter__p5 = CAST(1 AS bit) OR [p2].[IsDeleted] = CAST(0 AS bit) OR [p2].[IsDeleted] = CAST(0 AS bit))
    ) AS [p3] ON [s0].[SupplierID] = [p3].[SupplierID]
    WHERE @ef_filter__p7 = CAST(1 AS bit) OR [o].[IsDeleted] = CAST(0 AS bit) OR [o].[IsDeleted] = CAST(0 AS bit)
