-- SqlCe

SELECT
	CASE
		WHEN [x].[Kind] = 1 OR [x].[Kind] = 2 THEN 1
		ELSE 0
	END as [Color],
	[x].[Kind],
	Substring([x].[ItemCode], 1, 2) as [ItemCode],
	[x].[Color] as [Color_1],
	Substring([x].[ItemCode], 3, 2) as [ItemCode_1],
	[x].[Style] as [Style_1]
FROM
	[Item] [x]
WHERE
	([x].[Kind] = 1 OR [x].[Kind] = 2) AND NOT (([x].[Kind] = 1 OR [x].[Kind] = 2) AND [x].[Kind] = 1 OR NOT ([x].[Kind] = 1 OR [x].[Kind] = 2))

-- SqlCe

SELECT
	[t1].[ItemId],
	[t1].[Kind],
	[t1].[ItemCode],
	[t1].[Style] as [Style_1],
	[t1].[Color]
FROM
	[Item] [t1]

-- SqlCe

SELECT
	CASE
		WHEN [x].[Kind] = 1 OR [x].[Kind] = 3 THEN 1
		ELSE 0
	END as [Color],
	Substring([x].[ItemCode], 5, 2) as [ItemCode],
	[x].[Color] as [Color_1],
	[x].[Kind],
	Substring([x].[ItemCode], 7, 2) as [ItemCode_1],
	[x].[Style] as [Style_1]
FROM
	[Item] [x]
WHERE
	[x].[Kind] = 1 OR [x].[Kind] = 3

-- SqlCe

SELECT
	[t1].[ItemId],
	[t1].[Kind],
	[t1].[ItemCode],
	[t1].[Style] as [Style_1],
	[t1].[Color]
FROM
	[Item] [t1]

-- SqlCe

SELECT
	CASE
		WHEN [t].[Kind] = 1 OR [t].[Kind] = 2 THEN 1
		ELSE 0
	END as [c1],
	CASE
		WHEN [t].[Kind] = 1 THEN 1
		ELSE 0
	END as [Color],
	Substring([t].[ItemCode], 1, 2) as [ItemCode],
	[t].[Color] as [Name],
	NULL as [ItemCode_1],
	NULL as [Name_1],
	Substring([t].[ItemCode], 3, 2) as [ItemCode_2],
	[t].[Style] as [Name_2],
	NULL as [c2],
	NULL as [ItemCode_3],
	NULL as [Name_3]
FROM
	[Item] [t]
WHERE
	[t].[Kind] = 1 OR [t].[Kind] = 2
UNION ALL
SELECT
	CASE
		WHEN [t_1].[Kind] = 1 OR [t_1].[Kind] = 3 THEN 1
		ELSE 0
	END as [c1],
	NULL as [Color],
	NULL as [ItemCode],
	NULL as [Name],
	Substring([t_1].[ItemCode], 5, 2) as [ItemCode_1],
	[t_1].[Color] as [Name_1],
	NULL as [ItemCode_2],
	NULL as [Name_2],
	CASE
		WHEN [t_1].[Kind] = 1 THEN 1
		ELSE 0
	END as [c2],
	Substring([t_1].[ItemCode], 7, 2) as [ItemCode_3],
	[t_1].[Style] as [Name_3]
FROM
	[Item] [t_1]
WHERE
	[t_1].[Kind] = 1 OR [t_1].[Kind] = 3

-- SqlCe

SELECT
	[t1].[ItemId],
	[t1].[Kind],
	[t1].[ItemCode],
	[t1].[Style] as [Style_1],
	[t1].[Color]
FROM
	[Item] [t1]

-- SqlCe

SELECT
	[x_1].[Color],
	[x_1].[Color_1],
	[x_1].[ItemCode],
	[x_1].[Name],
	[x_1].[ItemCode_2] as [ItemCode_1],
	[x_1].[Name_2] as [Name_1],
	[x_1].[ItemCode_1] as [ItemCode_2],
	[x_1].[Name_1] as [Name_2],
	[x_1].[Size_1],
	[x_1].[ItemCode_3],
	[x_1].[Name_3]
FROM
	(
		SELECT
			CASE
				WHEN [x].[Kind] = 1 OR [x].[Kind] = 2 THEN 1
				ELSE 0
			END as [Color],
			CASE
				WHEN [x].[Kind] = 1 THEN 1
				ELSE 0
			END as [Color_1],
			Substring([x].[ItemCode], 1, 2) as [ItemCode],
			[x].[Color] as [Name],
			Substring([x].[ItemCode], 3, 2) as [ItemCode_1],
			[x].[Style] as [Name_1],
			NULL as [ItemCode_2],
			NULL as [Name_2],
			NULL as [Size_1],
			NULL as [ItemCode_3],
			NULL as [Name_3]
		FROM
			[Item] [x]
		WHERE
			[x].[Kind] = 1 OR [x].[Kind] = 2
		UNION ALL
		SELECT
			CASE
				WHEN [t].[Kind] = 1 OR [t].[Kind] = 3 THEN 1
				ELSE 0
			END as [Color],
			NULL as [Color_1],
			NULL as [ItemCode],
			NULL as [Name],
			NULL as [ItemCode_1],
			NULL as [Name_1],
			Substring([t].[ItemCode], 5, 2) as [ItemCode_2],
			[t].[Color] as [Name_2],
			CASE
				WHEN [t].[Kind] = 1 THEN 1
				ELSE 0
			END as [Size_1],
			Substring([t].[ItemCode], 7, 2) as [ItemCode_3],
			[t].[Style] as [Name_3]
		FROM
			[Item] [t]
		WHERE
			[t].[Kind] = 1 OR [t].[Kind] = 3
	) [x_1]
WHERE
	NOT ([x_1].[Color] = 1 AND [x_1].[Color_1] IS NOT NULL AND [x_1].[Color_1] = 1 OR [x_1].[Color] = 0)

-- SqlCe

SELECT
	[t1].[ItemId],
	[t1].[Kind],
	[t1].[ItemCode],
	[t1].[Style] as [Style_1],
	[t1].[Color]
FROM
	[Item] [t1]

-- SqlCe

SELECT
	[x_1].[Color],
	[x_1].[Color_1],
	[x_1].[ItemCode],
	[x_1].[Name],
	[x_1].[ItemCode_2] as [ItemCode_1],
	[x_1].[Name_2] as [Name_1],
	[x_1].[ItemCode_1] as [ItemCode_2],
	[x_1].[Name_1] as [Name_2],
	[x_1].[Size_1],
	[x_1].[ItemCode_3],
	[x_1].[Name_3]
FROM
	(
		SELECT
			CASE
				WHEN [x].[Kind] = 1 OR [x].[Kind] = 2 THEN 1
				ELSE 0
			END as [Color],
			CASE
				WHEN [x].[Kind] = 1 THEN 1
				ELSE 0
			END as [Color_1],
			Substring([x].[ItemCode], 1, 2) as [ItemCode],
			[x].[Color] as [Name],
			Substring([x].[ItemCode], 3, 2) as [ItemCode_1],
			[x].[Style] as [Name_1],
			NULL as [ItemCode_2],
			NULL as [Name_2],
			NULL as [Size_1],
			NULL as [ItemCode_3],
			NULL as [Name_3]
		FROM
			[Item] [x]
		WHERE
			[x].[Kind] = 1 OR [x].[Kind] = 2
		UNION ALL
		SELECT
			CASE
				WHEN [t].[Kind] = 1 OR [t].[Kind] = 3 THEN 1
				ELSE 0
			END as [Color],
			NULL as [Color_1],
			NULL as [ItemCode],
			NULL as [Name],
			NULL as [ItemCode_1],
			NULL as [Name_1],
			Substring([t].[ItemCode], 5, 2) as [ItemCode_2],
			[t].[Color] as [Name_2],
			CASE
				WHEN [t].[Kind] = 1 THEN 1
				ELSE 0
			END as [Size_1],
			Substring([t].[ItemCode], 7, 2) as [ItemCode_3],
			[t].[Style] as [Name_3]
		FROM
			[Item] [t]
		WHERE
			[t].[Kind] = 1 OR [t].[Kind] = 3
	) [x_1]
WHERE
	[x_1].[Color] = 1 AND [x_1].[Color_1] = 1 OR [x_1].[Color] = 0

-- SqlCe

SELECT
	[t1].[ItemId],
	[t1].[Kind],
	[t1].[ItemCode],
	[t1].[Style] as [Style_1],
	[t1].[Color]
FROM
	[Item] [t1]

-- SqlCe

SELECT
	[x_1].[Size_1],
	[x_1].[Color],
	[x_1].[ItemCode],
	[x_1].[Name],
	[x_1].[ItemCode_2] as [ItemCode_1],
	[x_1].[Name_2] as [Name_1],
	[x_1].[ItemCode_1] as [ItemCode_2],
	[x_1].[Name_1] as [Name_2],
	[x_1].[Size_2],
	[x_1].[ItemCode_3],
	[x_1].[Name_3]
FROM
	(
		SELECT
			CASE
				WHEN [x].[Kind] = 1 OR [x].[Kind] = 2 THEN 1
				ELSE 0
			END as [Size_1],
			CASE
				WHEN [x].[Kind] = 1 THEN 1
				ELSE 0
			END as [Color],
			Substring([x].[ItemCode], 1, 2) as [ItemCode],
			[x].[Color] as [Name],
			Substring([x].[ItemCode], 3, 2) as [ItemCode_1],
			[x].[Style] as [Name_1],
			NULL as [ItemCode_2],
			NULL as [Name_2],
			NULL as [Size_2],
			NULL as [ItemCode_3],
			NULL as [Name_3]
		FROM
			[Item] [x]
		WHERE
			[x].[Kind] = 1 OR [x].[Kind] = 2
		UNION ALL
		SELECT
			CASE
				WHEN [t].[Kind] = 1 OR [t].[Kind] = 3 THEN 1
				ELSE 0
			END as [Size_1],
			NULL as [Color],
			NULL as [ItemCode],
			NULL as [Name],
			NULL as [ItemCode_1],
			NULL as [Name_1],
			Substring([t].[ItemCode], 5, 2) as [ItemCode_2],
			[t].[Color] as [Name_2],
			CASE
				WHEN [t].[Kind] = 1 THEN 1
				ELSE 0
			END as [Size_2],
			Substring([t].[ItemCode], 7, 2) as [ItemCode_3],
			[t].[Style] as [Name_3]
		FROM
			[Item] [t]
		WHERE
			[t].[Kind] = 1 OR [t].[Kind] = 3
	) [x_1]
WHERE
	NOT ([x_1].[Size_1] = 1 AND [x_1].[Color] IS NULL AND [x_1].[Size_2] = 1 OR [x_1].[Size_1] = 0)

-- SqlCe

SELECT
	[t1].[ItemId],
	[t1].[Kind],
	[t1].[ItemCode],
	[t1].[Style] as [Style_1],
	[t1].[Color]
FROM
	[Item] [t1]

-- SqlCe

SELECT
	[x_1].[Size_1],
	[x_1].[Color],
	[x_1].[ItemCode],
	[x_1].[Name],
	[x_1].[ItemCode_2] as [ItemCode_1],
	[x_1].[Name_2] as [Name_1],
	[x_1].[ItemCode_1] as [ItemCode_2],
	[x_1].[Name_1] as [Name_2],
	[x_1].[Size_2],
	[x_1].[ItemCode_3],
	[x_1].[Name_3]
FROM
	(
		SELECT
			CASE
				WHEN [x].[Kind] = 1 OR [x].[Kind] = 2 THEN 1
				ELSE 0
			END as [Size_1],
			CASE
				WHEN [x].[Kind] = 1 THEN 1
				ELSE 0
			END as [Color],
			Substring([x].[ItemCode], 1, 2) as [ItemCode],
			[x].[Color] as [Name],
			Substring([x].[ItemCode], 3, 2) as [ItemCode_1],
			[x].[Style] as [Name_1],
			NULL as [ItemCode_2],
			NULL as [Name_2],
			NULL as [Size_2],
			NULL as [ItemCode_3],
			NULL as [Name_3]
		FROM
			[Item] [x]
		WHERE
			[x].[Kind] = 1 OR [x].[Kind] = 2
		UNION ALL
		SELECT
			CASE
				WHEN [t].[Kind] = 1 OR [t].[Kind] = 3 THEN 1
				ELSE 0
			END as [Size_1],
			NULL as [Color],
			NULL as [ItemCode],
			NULL as [Name],
			NULL as [ItemCode_1],
			NULL as [Name_1],
			Substring([t].[ItemCode], 5, 2) as [ItemCode_2],
			[t].[Color] as [Name_2],
			CASE
				WHEN [t].[Kind] = 1 THEN 1
				ELSE 0
			END as [Size_2],
			Substring([t].[ItemCode], 7, 2) as [ItemCode_3],
			[t].[Style] as [Name_3]
		FROM
			[Item] [t]
		WHERE
			[t].[Kind] = 1 OR [t].[Kind] = 3
	) [x_1]
WHERE
	[x_1].[Size_1] = 1 AND [x_1].[Color] IS NULL AND [x_1].[Size_2] = 1 OR
	[x_1].[Size_1] = 0

-- SqlCe

SELECT
	[t1].[ItemId],
	[t1].[Kind],
	[t1].[ItemCode],
	[t1].[Style] as [Style_1],
	[t1].[Color]
FROM
	[Item] [t1]

