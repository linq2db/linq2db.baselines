...
SELECT
	"r"."Date_1",
	CAST(LPad(CAST("r"."Date_Year" AS VarChar(4) CHARACTER SET UNICODE_FSS), 4, '0') || '-01-01' AS TimeStamp),
	CAST(LPad(CAST("r"."Date_Year" AS VarChar(4) CHARACTER SET UNICODE_FSS), 4, '0') || '-' || LPad(CAST("r"."Date_Month" AS VarChar(2) CHARACTER SET UNICODE_FSS), 2, '0') || '-01' AS TimeStamp)
FROM
	"x" "r"
