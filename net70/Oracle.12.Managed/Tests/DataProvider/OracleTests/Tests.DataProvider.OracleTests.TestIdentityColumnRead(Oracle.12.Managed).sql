...
BEGIN
	BEGIN
		EXECUTE IMMEDIATE 'DROP TRIGGER TIDENTITY_TEST_IDENTITY_SCHEMA';
	EXCEPTION
		WHEN OTHERS THEN
	...
	END;
	BEGIN
		EXECUTE IMMEDIATE 'DROP SEQUENCE SIDENTITY_TEST_IDENTITY_SCHEMA';
	EXCEPTION
		WHEN OTHERS THEN
	...
	END;
	BEGIN
		EXECUTE IMMEDIATE 'DROP TABLE TEST_IDENTITY_SCHEMA';
	EXCEPTION
		WHEN OTHERS THEN
			IF SQLCODE != -942 THEN
				RAISE;
...
BEGIN
	EXECUTE IMMEDIATE '
		CREATE TABLE TEST_IDENTITY_SCHEMA
		(
			ID     NUMBER GENERATED BY DEFAULT AS IDENTITY MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 1 NOCACHE ORDER NOCYCLE  NOT NULL,
			NOT_ID Int                                                                                                                                         NOT NULL
		)
	';
EXCEPTION
...
CREATE SEQUENCE SIDENTITY_TEST_IDENTITY_SCHEMA

BeforeExecute
-- Oracle.12.Managed Oracle.Managed Oracle12

CREATE OR REPLACE TRIGGER TIDENTITY_TEST_IDENTITY_SCHEMA
BEFORE INSERT ON TEST_IDENTITY_SCHEMA FOR EACH ROW
BEGIN
	SELECT SIDENTITY_TEST_IDENTITY_SCHEMA.NEXTVAL INTO :NEW.ID FROM dual;
END;
