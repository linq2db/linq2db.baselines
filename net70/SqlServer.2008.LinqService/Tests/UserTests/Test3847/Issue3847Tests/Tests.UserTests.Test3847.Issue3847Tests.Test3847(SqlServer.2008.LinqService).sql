...
FROM
	[OutfeedTransportOrderDTO] [outfeed]
		OUTER APPLY (
			...
			FROM
				(VALUES
					('bc7b663d-0fde-4327-8f92-5d8cc3a11d11',CAST('2020-02-29T17:54:55.0000000' AS DATETIME2))
				) [x]([Key], [Value])
			WHERE
				[x].[Key] = [outfeed].[Id]
		) [t1]
		OUTER APPLY (
			...
			FROM
				(VALUES
					('a948600d-de21-4f74-8ac2-9516b287076e',CAST('2020-02-29T17:54:55.0000000' AS DATETIME2))
				) [x_1]([Key], [Value])
			WHERE
				[x_1].[Key] = [outfeed].[Id]
		) [t2]
