...
SELECT
	`r`.`Date_1`,
	STR_TO_DATE(CONCAT(CAST(`r`.`Date_Year` AS CHAR(4)), '-01-01 00:00:00.000'), '%Y-%m-%d %H:%i:%s.%f'),
	STR_TO_DATE(CONCAT(CAST(`r`.`Date_Year` AS CHAR(4)), '-', LPad(CAST(`r`.`Date_Month` AS CHAR(2)), 2, '0'), '-01 00:00:00.000'), '%Y-%m-%d %H:%i:%s.%f')
FROM
	`x` `r`
