		...
		LEFT JOIN LATERAL (
			...
			FROM
				(
					...
					ELSE 0
				END AS "Conditional", 1 AS "field1", 2 AS "field2", 3 AS "field3" FROM rdb$database
					UNION ALL
					SELECT NULL, 0, 1, "a_Style"."Name", NULL, 4, 5, 6 FROM rdb$database) "it"
		) "it_1" ON 1=1
WHERE
...
FROM
	"SomeItem" "t3"
		...
		LEFT JOIN LATERAL (
			...
			WHEN "a_Color"."Name" = 'Red' THEN "t2".COUNT_1
			ELSE 0
		END AS "Conditional" FROM rdb$database
			UNION ALL
			SELECT NULL, 0, NULL FROM rdb$database) "it" ON 1=1
WHERE
