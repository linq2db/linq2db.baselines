...
$CTE_1 = 	SELECT
		t1.Id as Id,
		t1.Label as Label,
		ROW_NUMBER() OVER (ORDER BY t1.Id, t1.Label) as field,
		t1.Label = 'ActiveOnly'u as field_1
	FROM
		(
			SELECT
				Unwrap(CAST('ActiveOnly'u AS Text)) as Label,
				c_1.Id as Id
			FROM
				Company c_1
			UNION ALL
			SELECT
				Unwrap(CAST('All'u AS Text)) as Label,
				c_2.Id as Id
			FROM
				Company c_2
		) t1
...
SELECT
	...
	t7.Item6 as Item6,
	t7.Item7 as Item7,
	t7.Item1_2 as Item1_1,
	t7.Item1_1 as Item1_2,
	t7.Item2 as Item2
FROM
	(
		SELECT
