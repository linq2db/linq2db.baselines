...
SELECT
	a_MiddleRuntime_1.ParentID as ParentID,
...
FROM
	Parent t
		LEFT JOIN (
			SELECT
				a_MiddleRuntime.ParentID as ParentID,
				a_MiddleRuntime.ChildID as ChildID
			FROM
				Child a_MiddleRuntime
			WHERE
				a_MiddleRuntime.ChildID > 1
		) a_MiddleRuntime_1 ON t.ParentID = a_MiddleRuntime_1.ParentID
