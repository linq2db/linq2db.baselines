...
SELECT
	...
	CUME_DIST() OVER (ORDER BY x.`Timestamp`) as rn1,
	CUME_DIST() OVER (ORDER BY x.`Value`) as rn2,
	CUME_DIST() OVER (ORDER BY x.`Timestamp` DESC) as rn3,
	CUME_DIST() OVER (ORDER BY x.`Value` DESC) as rn4,
	CUME_DIST() OVER (ORDER BY x.`Timestamp`, x.`Value`) as rn5,
	CUME_DIST() OVER (ORDER BY x.`Timestamp` DESC, x.`Value` DESC) as rn6
FROM
