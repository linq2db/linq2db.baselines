...
SELECT
	...
	RANK() OVER (ORDER BY x.`Timestamp`) as rn1,
	RANK() OVER (ORDER BY x.`Value`) as rn2,
	RANK() OVER (ORDER BY x.`Timestamp` DESC) as rn3,
	RANK() OVER (ORDER BY x.`Value` DESC) as rn4,
	RANK() OVER (ORDER BY x.`Timestamp`, x.`Value`) as rn5,
	RANK() OVER (ORDER BY x.`Timestamp` DESC, x.`Value` DESC) as rn6
FROM
