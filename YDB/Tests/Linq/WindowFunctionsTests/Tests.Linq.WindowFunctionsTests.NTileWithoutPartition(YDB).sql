...
SELECT
	...
	NTILE(4) OVER (ORDER BY x.`Timestamp`) as nt1,
	NTILE(4) OVER (ORDER BY x.`Value`) as nt2,
	NTILE(4) OVER (ORDER BY x.`Timestamp` DESC) as nt3,
	NTILE(4) OVER (ORDER BY x.`Value` DESC) as nt4,
	NTILE(4) OVER (ORDER BY x.`Timestamp`, x.`Value`) as nt5,
	NTILE(4) OVER (ORDER BY x.`Timestamp` DESC, x.`Value` DESC) as nt6
FROM
