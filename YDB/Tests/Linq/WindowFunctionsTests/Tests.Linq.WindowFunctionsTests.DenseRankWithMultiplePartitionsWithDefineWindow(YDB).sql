...
SELECT
	...
	DENSE_RANK() OVER (PARTITION BY x.CategoryId ORDER BY x.`Timestamp`) as rn1,
	DENSE_RANK() OVER (PARTITION BY x.CategoryId, x.Name ORDER BY x.`Value`) as rn2,
	DENSE_RANK() OVER (PARTITION BY x.CategoryId, x.Name ORDER BY x.`Timestamp` DESC) as rn3,
	DENSE_RANK() OVER (PARTITION BY x.CategoryId, x.Name ORDER BY x.`Value` DESC) as rn4,
	DENSE_RANK() OVER (PARTITION BY x.CategoryId, x.Name ORDER BY x.`Timestamp`, x.`Value`) as rn5,
	DENSE_RANK() OVER (PARTITION BY x.CategoryId, x.Name ORDER BY x.`Timestamp` DESC, x.`Value` DESC) as rn6
FROM
