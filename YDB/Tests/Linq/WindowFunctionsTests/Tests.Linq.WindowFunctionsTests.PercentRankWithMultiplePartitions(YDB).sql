...
SELECT
	...
	PERCENT_RANK() OVER (PARTITION BY x.CategoryId, x.Name ORDER BY x.`Timestamp`) as rn1,
	PERCENT_RANK() OVER (PARTITION BY x.CategoryId, x.Name ORDER BY x.`Value`) as rn2,
	PERCENT_RANK() OVER (PARTITION BY x.CategoryId, x.Name ORDER BY x.`Timestamp` DESC) as rn3,
	PERCENT_RANK() OVER (PARTITION BY x.CategoryId, x.Name ORDER BY x.`Value` DESC) as rn4,
	PERCENT_RANK() OVER (PARTITION BY x.CategoryId, x.Name ORDER BY x.`Timestamp`, x.`Value`) as rn5,
	PERCENT_RANK() OVER (PARTITION BY x.CategoryId, x.Name ORDER BY x.`Timestamp` DESC, x.`Value` DESC) as rn6
FROM
