...
EXECUTE BLOCK AS BEGIN
	IF (EXISTS(SELECT 1 FROM rdb$triggers WHERE rdb$trigger_name = 'TIDENTITY_billing_DevReadingType')) THEN
		EXECUTE STATEMENT 'DROP TRIGGER "TIDENTITY_billing_DevReadingType"';
	IF (EXISTS(SELECT 1 FROM rdb$generators WHERE rdb$generator_name = 'GIDENTITY_billing_DevReadingType')) THEN
		EXECUTE STATEMENT 'DROP GENERATOR "GIDENTITY_billing_DevReadingType"';
	IF (EXISTS(SELECT 1 FROM rdb$relations WHERE rdb$relation_name = 'billing_DevReadingType')) THEN
		EXECUTE STATEMENT 'DROP TABLE "billing_DevReadingType"';
END
...
EXECUTE BLOCK AS BEGIN
	IF (NOT EXISTS(SELECT 1 FROM rdb$relations WHERE rdb$relation_name = 'billing_DevReadingType')) THEN
		EXECUTE STATEMENT '
		...
		';
	IF (NOT EXISTS(SELECT 1 FROM rdb$generators WHERE rdb$generator_name = 'GIDENTITY_billing_DevReadingType')) THEN
		EXECUTE STATEMENT '
			CREATE GENERATOR "GIDENTITY_billing_DevReadingType"
		';
	IF (NOT EXISTS(SELECT 1 FROM rdb$triggers WHERE rdb$trigger_name = 'TIDENTITY_billing_DevReadingType')) THEN
		EXECUTE STATEMENT '
			CREATE TRIGGER "TIDENTITY_billing_DevReadingType" FOR "billing_DevReadingType"
			BEFORE INSERT POSITION 0
			AS BEGIN
				NEW."Id" = GEN_ID("GIDENTITY_billing_DevReadingType", 1);
			END
		';
END
	...
	"DevReadingTypeId" = (
		...
		FROM
			"billing_TempReading" "tr_1"
				INNER JOIN "billing_DevReadingType" "drt_1" ON "drt_1"."Name" = "tr_1"."ReadingTypeName" AND "tr_1"."ReadingTypeName" IS NOT NULL AND "drt_1"."DevTypeId" = "tr_1"."Devtypeid" AND "tr_1"."Devtypeid" IS NOT NULL
		WHERE
	...
	"Responsibility" = (
		...
		FROM
			"billing_TempReading" "tr_2"
				INNER JOIN "billing_DevReadingType" "drt_2" ON "drt_2"."Name" = "tr_2"."ReadingTypeName" AND "tr_2"."ReadingTypeName" IS NOT NULL AND "drt_2"."DevTypeId" = "tr_2"."Devtypeid" AND "tr_2"."Devtypeid" IS NOT NULL
		WHERE
