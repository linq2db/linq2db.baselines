...
LEFT JOIN (
    ...
    INNER JOIN (
        ...
        WHERE @__ef_filter__p_4 = CAST(1 AS bit) OR [t].[IsDeleted] = CAST(0 AS bit) OR [t].[IsDeleted] = CAST(0 AS bit)
    ) AS [t0] ON [e1].[TerritoryID] = [t0].[TerritoryID]
    WHERE @__ef_filter__p_2 = CAST(1 AS bit) OR [e1].[IsDeleted] = CAST(0 AS bit) OR [e1].[IsDeleted] = CAST(0 AS bit)
