...
FROM
	(
		SELECT
			Cast(Floor(Extract(year from Coalesce("n".DATUM, Cast(@_default as TimeStamp)))) as int) as "Key_1",
			Cast(Floor(Extract(month from Coalesce("n".DATUM, Cast(@_default as TimeStamp)))) as int) as "Key_2",
			"n".SKUPAJ
		FROM
			"Issue3761Table" "n"
		WHERE
