-- Access.Jet.Odbc AccessODBC
SELECT bitDataType FROM [AllTypes] WHERE ID = 1

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 Bit -- Boolean
SET     @p0 = False
DECLARE @p1 Bit -- Boolean
SET     @p1 = False
DECLARE @p2 Bit -- Boolean
SET     @p2 = False

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND bitDataType IS NULL OR ? IS NOT NULL AND bitDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 Bit -- Boolean
SET     @p0 = False
DECLARE @p1 Bit -- Boolean
SET     @p1 = False
DECLARE @p2 Bit -- Boolean
SET     @p2 = False

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND bitDataType IS NULL OR ? IS NOT NULL AND bitDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT bitDataType FROM [AllTypes] WHERE ID = 2

-- Access.Jet.Odbc AccessODBC
DECLARE @p Bit -- Boolean
SET     @p = True

SELECT ID FROM [AllTypes] WHERE bitDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p Bit -- Boolean
SET     @p = True

SELECT ID FROM [AllTypes] WHERE bitDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p Bit -- Boolean
SET     @p = True

SELECT ID FROM [AllTypes] WHERE bitDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT smallintDataType FROM [AllTypes] WHERE ID = 1

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 SmallInt -- Int16
SET     @p0 = NULL
DECLARE @p1 SmallInt -- Int16
SET     @p1 = NULL
DECLARE @p2 SmallInt -- Int16
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND smallintDataType IS NULL OR ? IS NOT NULL AND smallintDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 NVarChar -- String
SET     @p0 = NULL
DECLARE @p1 NVarChar -- String
SET     @p1 = NULL
DECLARE @p2 NVarChar -- String
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND smallintDataType IS NULL OR ? IS NOT NULL AND smallintDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT smallintDataType FROM [AllTypes] WHERE ID = 2

-- Access.Jet.Odbc AccessODBC
DECLARE @p SmallInt -- Int16
SET     @p = 25555

SELECT ID FROM [AllTypes] WHERE smallintDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p SmallInt -- Int16
SET     @p = 25555

SELECT ID FROM [AllTypes] WHERE smallintDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p SmallInt -- Int16
SET     @p = 25555

SELECT ID FROM [AllTypes] WHERE smallintDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT decimalDataType FROM [AllTypes] WHERE ID = 1

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 VarChar -- AnsiString
SET     @p0 = NULL
DECLARE @p1 VarChar -- AnsiString
SET     @p1 = NULL
DECLARE @p2 VarChar -- AnsiString
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND decimalDataType IS NULL OR ? IS NOT NULL AND decimalDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 NVarChar -- String
SET     @p0 = NULL
DECLARE @p1 NVarChar -- String
SET     @p1 = NULL
DECLARE @p2 NVarChar -- String
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND decimalDataType IS NULL OR ? IS NOT NULL AND decimalDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT decimalDataType FROM [AllTypes] WHERE ID = 2

-- Access.Jet.Odbc AccessODBC
DECLARE @p VarChar(7, 0) -- AnsiString
SET     @p = 2222222

SELECT ID FROM [AllTypes] WHERE decimalDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p VarChar(18, 10) -- AnsiString
SET     @p = 2222222

SELECT ID FROM [AllTypes] WHERE decimalDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p VarChar(7, 0) -- AnsiString
SET     @p = 2222222

SELECT ID FROM [AllTypes] WHERE decimalDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT intDataType FROM [AllTypes] WHERE ID = 1

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 Int -- Int32
SET     @p0 = NULL
DECLARE @p1 Int -- Int32
SET     @p1 = NULL
DECLARE @p2 Int -- Int32
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND intDataType IS NULL OR ? IS NOT NULL AND intDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 NVarChar -- String
SET     @p0 = NULL
DECLARE @p1 NVarChar -- String
SET     @p1 = NULL
DECLARE @p2 NVarChar -- String
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND intDataType IS NULL OR ? IS NOT NULL AND intDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT intDataType FROM [AllTypes] WHERE ID = 2

-- Access.Jet.Odbc AccessODBC
DECLARE @p Int -- Int32
SET     @p = 7777777

SELECT ID FROM [AllTypes] WHERE intDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p Int -- Int32
SET     @p = 7777777

SELECT ID FROM [AllTypes] WHERE intDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p Int -- Int32
SET     @p = 7777777

SELECT ID FROM [AllTypes] WHERE intDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT tinyintDataType FROM [AllTypes] WHERE ID = 1

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 TinyInt -- Byte
SET     @p0 = NULL
DECLARE @p1 TinyInt -- Byte
SET     @p1 = NULL
DECLARE @p2 TinyInt -- Byte
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND tinyintDataType IS NULL OR ? IS NOT NULL AND tinyintDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 NVarChar -- String
SET     @p0 = NULL
DECLARE @p1 NVarChar -- String
SET     @p1 = NULL
DECLARE @p2 NVarChar -- String
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND tinyintDataType IS NULL OR ? IS NOT NULL AND tinyintDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT tinyintDataType FROM [AllTypes] WHERE ID = 2

-- Access.Jet.Odbc AccessODBC
DECLARE @p TinyInt(1) -- Byte
SET     @p = 100

SELECT ID FROM [AllTypes] WHERE tinyintDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p TinyInt(1) -- Byte
SET     @p = 100

SELECT ID FROM [AllTypes] WHERE tinyintDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p TinyInt(1) -- Byte
SET     @p = 100

SELECT ID FROM [AllTypes] WHERE tinyintDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT moneyDataType FROM [AllTypes] WHERE ID = 1

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 VarChar -- AnsiString
SET     @p0 = NULL
DECLARE @p1 VarChar -- AnsiString
SET     @p1 = NULL
DECLARE @p2 VarChar -- AnsiString
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND moneyDataType IS NULL OR ? IS NOT NULL AND moneyDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 NVarChar -- String
SET     @p0 = NULL
DECLARE @p1 NVarChar -- String
SET     @p1 = NULL
DECLARE @p2 NVarChar -- String
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND moneyDataType IS NULL OR ? IS NOT NULL AND moneyDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT moneyDataType FROM [AllTypes] WHERE ID = 2

-- Access.Jet.Odbc AccessODBC
DECLARE @p VarChar(10, 4) -- AnsiString
SET     @p = 100000.0000

SELECT ID FROM [AllTypes] WHERE moneyDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p VarChar(18, 10) -- AnsiString
SET     @p = 100000.0000

SELECT ID FROM [AllTypes] WHERE moneyDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p VarChar(10, 4) -- AnsiString
SET     @p = 100000.0000

SELECT ID FROM [AllTypes] WHERE moneyDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT floatDataType FROM [AllTypes] WHERE ID = 1

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 Double
SET     @p0 = NULL
DECLARE @p1 Double
SET     @p1 = NULL
DECLARE @p2 Double
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND floatDataType IS NULL OR ? IS NOT NULL AND floatDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 NVarChar -- String
SET     @p0 = NULL
DECLARE @p1 NVarChar -- String
SET     @p1 = NULL
DECLARE @p2 NVarChar -- String
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND floatDataType IS NULL OR ? IS NOT NULL AND floatDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT floatDataType FROM [AllTypes] WHERE ID = 2

-- Access.Jet.Odbc AccessODBC
DECLARE @p Double
SET     @p = 20.309999999999999

SELECT ID FROM [AllTypes] WHERE floatDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p Double
SET     @p = 20.309999999999999

SELECT ID FROM [AllTypes] WHERE floatDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p Double
SET     @p = 20.309999999999999

SELECT ID FROM [AllTypes] WHERE floatDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT realDataType FROM [AllTypes] WHERE ID = 1

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 Real -- Single
SET     @p0 = NULL
DECLARE @p1 Real -- Single
SET     @p1 = NULL
DECLARE @p2 Real -- Single
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND realDataType IS NULL OR ? IS NOT NULL AND realDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 NVarChar -- String
SET     @p0 = NULL
DECLARE @p1 NVarChar -- String
SET     @p1 = NULL
DECLARE @p2 NVarChar -- String
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND realDataType IS NULL OR ? IS NOT NULL AND realDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT realDataType FROM [AllTypes] WHERE ID = 2

-- Access.Jet.Odbc AccessODBC
DECLARE @p Real -- Single
SET     @p = 16.2000008

SELECT ID FROM [AllTypes] WHERE realDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p Real -- Single
SET     @p = 16.2000008

SELECT ID FROM [AllTypes] WHERE realDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p Real -- Single
SET     @p = 16.2000008

SELECT ID FROM [AllTypes] WHERE realDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT datetimeDataType FROM [AllTypes] WHERE ID = 1

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 DateTime
SET     @p0 = NULL
DECLARE @p1 DateTime
SET     @p1 = NULL
DECLARE @p2 DateTime
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND datetimeDataType IS NULL OR ? IS NOT NULL AND datetimeDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 NVarChar -- String
SET     @p0 = NULL
DECLARE @p1 NVarChar -- String
SET     @p1 = NULL
DECLARE @p2 NVarChar -- String
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND datetimeDataType IS NULL OR ? IS NOT NULL AND datetimeDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT datetimeDataType FROM [AllTypes] WHERE ID = 2

-- Access.Jet.Odbc AccessODBC
DECLARE @p DateTime
SET     @p = #2012-12-12 12:12:12#

SELECT ID FROM [AllTypes] WHERE datetimeDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p DateTime
SET     @p = #2012-12-12 12:12:12#

SELECT ID FROM [AllTypes] WHERE datetimeDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p DateTime
SET     @p = #2012-12-12 12:12:12#

SELECT ID FROM [AllTypes] WHERE datetimeDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT charDataType FROM [AllTypes] WHERE ID = 1

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 Char -- AnsiStringFixedLength
SET     @p0 = NULL
DECLARE @p1 Char -- AnsiStringFixedLength
SET     @p1 = NULL
DECLARE @p2 Char -- AnsiStringFixedLength
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND charDataType IS NULL OR ? IS NOT NULL AND charDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 NVarChar -- String
SET     @p0 = NULL
DECLARE @p1 NVarChar -- String
SET     @p1 = NULL
DECLARE @p2 NVarChar -- String
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND charDataType IS NULL OR ? IS NOT NULL AND charDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT charDataType FROM [AllTypes] WHERE ID = 2

-- Access.Jet.Odbc AccessODBC
DECLARE @p Char(1) -- AnsiStringFixedLength
SET     @p = '1'

SELECT ID FROM [AllTypes] WHERE charDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p NChar(1) -- StringFixedLength
SET     @p = '1'

SELECT ID FROM [AllTypes] WHERE charDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p NChar(1) -- StringFixedLength
SET     @p = '1'

SELECT ID FROM [AllTypes] WHERE charDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT varcharDataType FROM [AllTypes] WHERE ID = 1

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 VarChar -- AnsiString
SET     @p0 = NULL
DECLARE @p1 VarChar -- AnsiString
SET     @p1 = NULL
DECLARE @p2 VarChar -- AnsiString
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND varcharDataType IS NULL OR ? IS NOT NULL AND varcharDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 NVarChar -- String
SET     @p0 = NULL
DECLARE @p1 NVarChar -- String
SET     @p1 = NULL
DECLARE @p2 NVarChar -- String
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND varcharDataType IS NULL OR ? IS NOT NULL AND varcharDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT varcharDataType FROM [AllTypes] WHERE ID = 2

-- Access.Jet.Odbc AccessODBC
DECLARE @p VarChar(3) -- AnsiString
SET     @p = '234'

SELECT ID FROM [AllTypes] WHERE varcharDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p NVarChar(255) -- String
SET     @p = '234'

SELECT ID FROM [AllTypes] WHERE varcharDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p NVarChar(3) -- String
SET     @p = '234'

SELECT ID FROM [AllTypes] WHERE varcharDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT textDataType FROM [AllTypes] WHERE ID = 1

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 NVarChar -- String
SET     @p0 = NULL
DECLARE @p1 NVarChar -- String
SET     @p1 = NULL
DECLARE @p2 NVarChar -- String
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND textDataType IS NULL OR ? IS NOT NULL AND textDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 NVarChar -- String
SET     @p0 = NULL
DECLARE @p1 NVarChar -- String
SET     @p1 = NULL
DECLARE @p2 NVarChar -- String
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND textDataType IS NULL OR ? IS NOT NULL AND textDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT textDataType FROM [AllTypes] WHERE ID = 2

-- Access.Jet.Odbc AccessODBC
DECLARE @p NVarChar(3) -- String
SET     @p = '567'

SELECT ID FROM [AllTypes] WHERE textDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p NVarChar(255) -- String
SET     @p = '567'

SELECT ID FROM [AllTypes] WHERE textDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p NVarChar(3) -- String
SET     @p = '567'

SELECT ID FROM [AllTypes] WHERE textDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT ncharDataType FROM [AllTypes] WHERE ID = 1

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 NChar -- StringFixedLength
SET     @p0 = NULL
DECLARE @p1 NChar -- StringFixedLength
SET     @p1 = NULL
DECLARE @p2 NChar -- StringFixedLength
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND ncharDataType IS NULL OR ? IS NOT NULL AND ncharDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 NVarChar -- String
SET     @p0 = NULL
DECLARE @p1 NVarChar -- String
SET     @p1 = NULL
DECLARE @p2 NVarChar -- String
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND ncharDataType IS NULL OR ? IS NOT NULL AND ncharDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT ncharDataType FROM [AllTypes] WHERE ID = 2

-- Access.Jet.Odbc AccessODBC
DECLARE @p NChar(5) -- StringFixedLength
SET     @p = '23233'

SELECT ID FROM [AllTypes] WHERE ncharDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p NVarChar(255) -- String
SET     @p = '23233'

SELECT ID FROM [AllTypes] WHERE ncharDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p NVarChar(5) -- String
SET     @p = '23233'

SELECT ID FROM [AllTypes] WHERE ncharDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT nvarcharDataType FROM [AllTypes] WHERE ID = 1

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 NVarChar -- String
SET     @p0 = NULL
DECLARE @p1 NVarChar -- String
SET     @p1 = NULL
DECLARE @p2 NVarChar -- String
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND nvarcharDataType IS NULL OR ? IS NOT NULL AND nvarcharDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 NVarChar -- String
SET     @p0 = NULL
DECLARE @p1 NVarChar -- String
SET     @p1 = NULL
DECLARE @p2 NVarChar -- String
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND nvarcharDataType IS NULL OR ? IS NOT NULL AND nvarcharDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT nvarcharDataType FROM [AllTypes] WHERE ID = 2

-- Access.Jet.Odbc AccessODBC
DECLARE @p NVarChar(4) -- String
SET     @p = '3323'

SELECT ID FROM [AllTypes] WHERE nvarcharDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p NVarChar(255) -- String
SET     @p = '3323'

SELECT ID FROM [AllTypes] WHERE nvarcharDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p NVarChar(4) -- String
SET     @p = '3323'

SELECT ID FROM [AllTypes] WHERE nvarcharDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT ntextDataType FROM [AllTypes] WHERE ID = 1

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 NVarChar -- String
SET     @p0 = NULL
DECLARE @p1 NVarChar -- String
SET     @p1 = NULL
DECLARE @p2 NVarChar -- String
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND ntextDataType IS NULL OR ? IS NOT NULL AND ntextDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 NVarChar -- String
SET     @p0 = NULL
DECLARE @p1 NVarChar -- String
SET     @p1 = NULL
DECLARE @p2 NVarChar -- String
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND ntextDataType IS NULL OR ? IS NOT NULL AND ntextDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT ntextDataType FROM [AllTypes] WHERE ID = 2

-- Access.Jet.Odbc AccessODBC
DECLARE @p NVarChar(3) -- String
SET     @p = '111'

SELECT ID FROM [AllTypes] WHERE ntextDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p NVarChar(255) -- String
SET     @p = '111'

SELECT ID FROM [AllTypes] WHERE ntextDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p NVarChar(3) -- String
SET     @p = '111'

SELECT ID FROM [AllTypes] WHERE ntextDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT binaryDataType FROM [AllTypes] WHERE ID = 1

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 VarBinary -- Binary
SET     @p0 = NULL
DECLARE @p1 VarBinary -- Binary
SET     @p1 = NULL
DECLARE @p2 VarBinary -- Binary
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND binaryDataType IS NULL OR ? IS NOT NULL AND binaryDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 NVarChar -- String
SET     @p0 = NULL
DECLARE @p1 NVarChar -- String
SET     @p1 = NULL
DECLARE @p2 NVarChar -- String
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND binaryDataType IS NULL OR ? IS NOT NULL AND binaryDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT binaryDataType FROM [AllTypes] WHERE ID = 2

-- Access.Jet.Odbc AccessODBC
DECLARE @p VarBinary(10) -- Binary
SET     @p = 0x01020304000000000000

SELECT ID FROM [AllTypes] WHERE binaryDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p VarBinary(10) -- Binary
SET     @p = 0x01020304000000000000

SELECT ID FROM [AllTypes] WHERE binaryDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p VarBinary(10) -- Binary
SET     @p = 0x01020304000000000000

SELECT ID FROM [AllTypes] WHERE binaryDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT varbinaryDataType FROM [AllTypes] WHERE ID = 1

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 VarBinary -- Binary
SET     @p0 = NULL
DECLARE @p1 VarBinary -- Binary
SET     @p1 = NULL
DECLARE @p2 VarBinary -- Binary
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND varbinaryDataType IS NULL OR ? IS NOT NULL AND varbinaryDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 NVarChar -- String
SET     @p0 = NULL
DECLARE @p1 NVarChar -- String
SET     @p1 = NULL
DECLARE @p2 NVarChar -- String
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND varbinaryDataType IS NULL OR ? IS NOT NULL AND varbinaryDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT varbinaryDataType FROM [AllTypes] WHERE ID = 2

-- Access.Jet.Odbc AccessODBC
DECLARE @p VarBinary(4) -- Binary
SET     @p = 0x01020305

SELECT ID FROM [AllTypes] WHERE varbinaryDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p VarBinary(4) -- Binary
SET     @p = 0x01020305

SELECT ID FROM [AllTypes] WHERE varbinaryDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p VarBinary(4) -- Binary
SET     @p = 0x01020305

SELECT ID FROM [AllTypes] WHERE varbinaryDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT imageDataType FROM [AllTypes] WHERE ID = 1

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 NVarChar -- String
SET     @p0 = NULL
DECLARE @p1 NVarChar -- String
SET     @p1 = NULL
DECLARE @p2 NVarChar -- String
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND imageDataType IS NULL OR ? IS NOT NULL AND imageDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 NVarChar -- String
SET     @p0 = NULL
DECLARE @p1 NVarChar -- String
SET     @p1 = NULL
DECLARE @p2 NVarChar -- String
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND imageDataType IS NULL OR ? IS NOT NULL AND imageDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT imageDataType FROM [AllTypes] WHERE ID = 2

-- Access.Jet.Odbc AccessODBC
DECLARE @p NVarChar(4) -- String
SET     @p = 0x03040506

SELECT ID FROM [AllTypes] WHERE imageDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p VarBinary(4) -- Binary
SET     @p = 0x03040506

SELECT ID FROM [AllTypes] WHERE imageDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p VarBinary(4) -- Binary
SET     @p = 0x03040506

SELECT ID FROM [AllTypes] WHERE imageDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT oleobjectDataType FROM [AllTypes] WHERE ID = 1

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 Binary
SET     @p0 = NULL
DECLARE @p1 Binary
SET     @p1 = NULL
DECLARE @p2 Binary
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND oleobjectDataType IS NULL OR ? IS NOT NULL AND oleobjectDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 NVarChar -- String
SET     @p0 = NULL
DECLARE @p1 NVarChar -- String
SET     @p1 = NULL
DECLARE @p2 NVarChar -- String
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND oleobjectDataType IS NULL OR ? IS NOT NULL AND oleobjectDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT oleobjectDataType FROM [AllTypes] WHERE ID = 2

-- Access.Jet.Odbc AccessODBC
DECLARE @p VarBinary(4) -- Binary
SET     @p = 0x05060708

SELECT ID FROM [AllTypes] WHERE oleobjectDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p VarBinary(4) -- Binary
SET     @p = 0x05060708

SELECT ID FROM [AllTypes] WHERE oleobjectDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT uniqueidentifierDataType FROM [AllTypes] WHERE ID = 1

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 UniqueIdentifier -- Guid
SET     @p0 = NULL
DECLARE @p1 UniqueIdentifier -- Guid
SET     @p1 = NULL
DECLARE @p2 UniqueIdentifier -- Guid
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND uniqueidentifierDataType IS NULL OR ? IS NOT NULL AND uniqueidentifierDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p0 NVarChar -- String
SET     @p0 = NULL
DECLARE @p1 NVarChar -- String
SET     @p1 = NULL
DECLARE @p2 NVarChar -- String
SET     @p2 = NULL

SELECT ID FROM [AllTypes] WHERE ? IS NULL AND uniqueidentifierDataType IS NULL OR ? IS NOT NULL AND uniqueidentifierDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
SELECT uniqueidentifierDataType FROM [AllTypes] WHERE ID = 2

-- Access.Jet.Odbc AccessODBC
DECLARE @p UniqueIdentifier -- Guid
SET     @p = '{6f9619ff-8b86-d011-b42d-00c04fc964ff}'

SELECT ID FROM [AllTypes] WHERE uniqueidentifierDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p UniqueIdentifier -- Guid
SET     @p = '{6f9619ff-8b86-d011-b42d-00c04fc964ff}'

SELECT ID FROM [AllTypes] WHERE uniqueidentifierDataType = ? ORDER BY ID

-- Access.Jet.Odbc AccessODBC
DECLARE @p UniqueIdentifier -- Guid
SET     @p = '{6f9619ff-8b86-d011-b42d-00c04fc964ff}'

SELECT ID FROM [AllTypes] WHERE uniqueidentifierDataType = ? ORDER BY ID

