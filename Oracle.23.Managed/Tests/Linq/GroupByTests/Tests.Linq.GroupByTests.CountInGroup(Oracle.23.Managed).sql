...
FROM
	(
		SELECT
			...
			COUNT(*) as COUNT_1,
			COUNT(CASE
				WHEN MOD(t."DataValue", 2) = 0D AND t."DataValue" IS NOT NULL
					THEN 1
				ELSE NULL
			END) as COUNT_2,
			COUNT(*) as COUNT_3,
		...
		OUTER APPLY (
			...
			WHERE
				MOD(x_1."DataValue", 2) = 0D AND x_1."DataValue" IS NOT NULL
		) t1
		OUTER APPLY (
			SELECT
				COUNT(*) as COUNT_1
		...
		OUTER APPLY (
			...
			WHERE
				MOD(x_3."DataValue", 2) = 0D AND x_3."DataValue" IS NOT NULL
		) t4
		OUTER APPLY (
			SELECT
				COUNT(*) as COUNT_1
