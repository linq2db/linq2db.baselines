	...
	(
		...
		FROM
			(
				...
				LEFT JOIN `Trade` `trade_9` ON `t8`.`AlertKey` = CAST(`trade_9`.`DealId` AS CHAR(11))
				LEFT JOIN `Nomin` `nomin_9` ON `t8`.`AlertKey` = CAST(`nomin_9`.`CargoId` AS CHAR(11))
		WHERE
			(LOCATE(@cpty, `nomin_9`.`DeliveryCounterParty`) > 0 OR LOCATE(@cpty, `trade_9`.`CounterParty`) > 0 OR LOCATE(@cpty, `t8`.`AlertCode`) > 0) AND
