...
LEFT JOIN (
    ...
    INNER JOIN (
        ...
        FROM [Products] AS [p]
        WHERE (@ef_filter__p5 = CAST(1 AS bit) OR [p].[ProductID] > 2) AND (@ef_filter__p5 = CAST(1 AS bit) OR [p].[Discontinued] = CAST(0 AS bit)) AND (@ef_filter__p1 = CAST(1 AS bit) OR [p].[IsDeleted] = CAST(0 AS bit) OR [p].[IsDeleted] = CAST(0 AS bit))
    ) AS [p0] ON [o0].[ProductID] = [p0].[ProductID]
    LEFT JOIN (
        SELECT [o1].[OrderID], [o1].[ProductID], [o1].[Discount], [o1].[IsDeleted], [o1].[Quantity], [o1].[UnitPrice]
        FROM [Order Details] AS [o1]
        WHERE @ef_filter__p1 = CAST(1 AS bit) OR [o1].[IsDeleted] = CAST(0 AS bit) OR [o1].[IsDeleted] = CAST(0 AS bit)
