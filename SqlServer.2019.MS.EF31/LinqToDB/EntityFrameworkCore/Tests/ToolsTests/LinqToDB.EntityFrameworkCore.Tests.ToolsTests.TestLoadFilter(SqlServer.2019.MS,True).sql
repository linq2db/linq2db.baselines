	...
	(
		...
		FROM
			(
				...
				LEFT JOIN [Suppliers] [a_Supplier] ON [a_Product].[SupplierID] = [a_Supplier].[SupplierID] AND [a_Product].[SupplierID] IS NOT NULL AND [a_Supplier].[IsDeleted] = 0
		WHERE
			[e_1].[IsDeleted] = 0 AND
...
FROM
	[Products] [m_1]
		...
		LEFT JOIN [Suppliers] [a_Supplier] ON [m_1].[SupplierID] = [a_Supplier].[SupplierID] AND [m_1].[SupplierID] IS NOT NULL AND [a_Supplier].[IsDeleted] = 0
WHERE
	[m_1].[IsDeleted] = 0 AND
	[e].[IsDeleted] = 0 AND
	[m_1].[IsDeleted] = 0 AND
