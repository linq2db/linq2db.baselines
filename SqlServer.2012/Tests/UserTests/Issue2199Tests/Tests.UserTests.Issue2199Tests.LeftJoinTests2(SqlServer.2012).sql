...
FROM
	[Manufacturer] [e]
		LEFT JOIN [Country] [co] ON [co].[Code] = [e].[CountryCode] AND [e].[CountryCode] IS NOT NULL
		LEFT JOIN (
			SELECT
				[m_1].[ManufacturerId],
				[m_1].[CountryCode],
			...
			FROM
				[Manufacturer] [m_1]
					LEFT JOIN [Country] [co_1] ON [co_1].[Code] = [m_1].[CountryCode] AND [m_1].[CountryCode] IS NOT NULL
			WHERE
