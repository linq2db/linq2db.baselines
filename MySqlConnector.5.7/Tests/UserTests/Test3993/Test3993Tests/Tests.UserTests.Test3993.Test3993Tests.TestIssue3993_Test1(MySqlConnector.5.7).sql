...
-- MySqlConnector.5.7 MySql.5.7.MySqlConnector MySql57
SELECT
	CAST(CAST(`x`.`PreNotification` AS SIGNED) AS DECIMAL(29, 10)) / 10000,
	CAST(CAST(`x`.`PreNotification` AS SIGNED) AS DECIMAL(29, 10)) / 10000000
FROM
	`Common_Topology_Locations` `x`
WHERE
	CAST(CAST(`x`.`PreNotification` AS SIGNED) AS DECIMAL(29, 10)) / 10000000 < CAST(CAST(`x`.`PreNotification` AS SIGNED) AS DECIMAL(29, 10)) / 10000
...
FROM
	(
		SELECT
			Date_Add(`t`.`StartDateTime`, Interval ((-1 * (CAST(CAST(`t`.`PreNotification` AS SIGNED) AS DECIMAL(29, 10)) / 10000)) * 1000) Microsecond) as `NotificationDateTime`,
			`t`.`StartDateTime`,
			`t`.`PreNotification`,
			Date_Add(`t`.`StartDateTime`, Interval ((-1 * (CAST(CAST(`t`.`PreNotification2` AS SIGNED) AS DECIMAL(29, 10)) / 10000)) * 1000) Microsecond) as `NotificationDateTime2`,
			Date_Add(`t`.`StartDateTime`, Interval ((-1 * (CAST(CAST(`t`.`PreNotification3` AS SIGNED) AS DECIMAL(29, 10)) / 10000)) * 1000) Microsecond) as `NotificationDateTime3`,
...
FROM
	`Common_Topology_Locations` `x`
WHERE
	CAST(CAST(TIMESTAMPDIFF(MICROSECOND, `x`.`StartDateTime`, `x`.`EndDateTime`) * 10 AS SIGNED) AS DECIMAL(29, 10)) / 36000000000 < 5
...
FROM
	`Common_Topology_Locations` `x`
WHERE
	CAST(CAST(TIMESTAMPDIFF(MICROSECOND, `x`.`StartDateTime`, `x`.`EndDateTime`) * 10 AS SIGNED) AS DECIMAL(29, 10)) / 36000000000 < 2
