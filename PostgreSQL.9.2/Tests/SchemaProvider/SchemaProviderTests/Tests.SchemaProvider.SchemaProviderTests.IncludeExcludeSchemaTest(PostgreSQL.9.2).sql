	...
	(
		...
		(
			...
			FROM pg_catalog.pg_class cls
				...
				LEFT JOIN
					(pg_type bt JOIN pg_namespace nbt ON bt.typnamespace = nbt.oid) ON typ.typtype = 'd'::"char" AND typ.typbasetype = bt.oid
			WHERE cls.relkind IN ('r', 'v', 'm') AND attr.attnum > 0 AND NOT attr.attisdropped AND ns.nspname NOT IN ('information_schema', 'pg_catalog', 'public', 'test_schema')
		) columns
	) columns
) columns;
	...
	(
		...
		(
			...
			FROM pg_catalog.pg_class cls
				...
				LEFT JOIN
					(pg_type bt JOIN pg_namespace nbt ON bt.typnamespace = nbt.oid) ON typ.typtype = 'd'::"char" AND typ.typbasetype = bt.oid
			WHERE cls.relkind IN ('r', 'v', 'm') AND attr.attnum > 0 AND NOT attr.attisdropped AND ns.nspname NOT IN ('information_schema', 'pg_catalog') AND ns.nspname IN ('IncludeExcludeSchemaTest')
		) columns
	) columns
) columns;
