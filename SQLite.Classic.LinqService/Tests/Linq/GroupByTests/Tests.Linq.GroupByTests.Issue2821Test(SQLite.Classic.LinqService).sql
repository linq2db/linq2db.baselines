	...
	[LinqDataTypes] [o]
		INNER JOIN (
			...
			WHERE
				...
				([t].[DateTimeValue2] IS NULL OR strftime('%Y-%m-%d %H:%M:%f', [t].[DateTimeValue2]) >= strftime('%Y-%m-%d %H:%M:%f', @currentDate))
			GROUP BY
				[t].[ID]
...
WHERE
	...
	([o].[DateTimeValue2] IS NULL OR strftime('%Y-%m-%d %H:%M:%f', [o].[DateTimeValue2]) >= strftime('%Y-%m-%d %H:%M:%f', @currentDate))
ORDER BY
	[o].[DateTimeValue2]
