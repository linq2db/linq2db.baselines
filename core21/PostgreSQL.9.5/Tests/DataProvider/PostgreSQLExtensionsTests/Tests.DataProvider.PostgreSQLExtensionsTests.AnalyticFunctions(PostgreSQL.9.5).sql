...
SELECT
	...
	ARRAY_AGG(v.*) FILTER (WHERE (v.* ILIKE 'V0%' ESCAPE '~')) OVER(PARTITION BY (t."Id" / 3)),
	ARRAY_AGG(ALL v.*) FILTER (WHERE (v.* ILIKE 'V0%' ESCAPE '~')) OVER(PARTITION BY (t."Id" / 3)),
	ARRAY_AGG(ALL v.*) FILTER (WHERE (v.* ILIKE 'V0%' ESCAPE '~')) OVER(PARTITION BY (t."Id" / 3) ORDER BY t."Id", (t."Id" - 1)),
