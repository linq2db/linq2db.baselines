...
SELECT
	IIF(N'Issue1977Table/' + Convert(NVarChar(36), [f].[firstField]) + N'/' + Convert(NVarChar(36), [f].[secondField]) = CONCAT_WS(N'/', N'Issue1977Table', Convert(NVarChar(36), [f].[firstField]), Convert(NVarChar(36), [f].[secondField])), 1, 0)
FROM
	[Issue1977Table] [f]
