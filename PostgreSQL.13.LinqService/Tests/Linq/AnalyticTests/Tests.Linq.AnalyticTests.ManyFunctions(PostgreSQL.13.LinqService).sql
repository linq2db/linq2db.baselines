...
SELECT
	...
	COUNT(*) OVER(),
	RANK() OVER(PARTITION BY p."Value1", c_1."ChildID" ORDER BY p."Value1", c_1."ChildID")::Float + Sqrt(DENSE_RANK() OVER(PARTITION BY p."Value1", c_1."ChildID" ORDER BY p."Value1")::Float) + COUNT(ALL p."ParentID") OVER(PARTITION BY p."Value1" ORDER BY p."Value1" RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) + COUNT(*) OVER()
FROM
	"Parent" p
