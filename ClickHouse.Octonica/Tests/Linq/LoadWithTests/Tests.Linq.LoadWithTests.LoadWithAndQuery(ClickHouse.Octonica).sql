...
FROM
	(
		...
		INNER JOIN SubItem1 i ON lw_MainItem.Id = i.ParentId AND i.Id % toInt32(2) = toInt32(0)
		INNER JOIN MainItem2 mm ON i.Id / toInt32(10) = mm.Id
...
FROM
	SubItem1 s
		LEFT JOIN MainItem p ON s.ParentId = p.Id AND p.Id % toInt32(3) = toInt32(0)
