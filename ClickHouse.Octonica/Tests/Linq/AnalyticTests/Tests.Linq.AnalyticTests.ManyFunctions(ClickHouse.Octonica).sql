...
SELECT
	...
	toInt64(ROW_NUMBER() OVER (PARTITION BY p.Value1, c_1.ChildID ORDER BY p.Value1 DESC, c_1.ChildID DESC, c_1.ParentID DESC)),
	...
	COUNT(p.ParentID) OVER (PARTITION BY p.Value1 ORDER BY p.Value1 RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW),
	COUNT(p.ParentID) OVER (PARTITION BY p.Value1 ORDER BY p.Value1 RANGE BETWEEN CURRENT ROW AND CURRENT ROW),
	COUNT(p.ParentID) OVER (PARTITION BY p.Value1 ORDER BY p.Value1 ROWS BETWEEN 1 PRECEDING AND CURRENT ROW),
	...
	COUNT(p.ParentID) OVER (PARTITION BY p.Value1 ORDER BY p.Value1 DESC RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW),
	COUNT(*) OVER (),
	toFloat64(toInt64(RANK() OVER (PARTITION BY p.Value1, c_1.ChildID ORDER BY p.Value1, c_1.ChildID))) + Sqrt(toFloat64(DENSE_RANK() OVER (PARTITION BY p.Value1, c_1.ChildID ORDER BY p.Value1))) + COUNT(p.ParentID) OVER (PARTITION BY p.Value1 ORDER BY p.Value1 RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) + COUNT(*) OVER ()
FROM
	Parent p
