...
SELECT
	...
	COUNT(p.ParentID) OVER (PARTITION BY p.Value1 ORDER BY p.Value1 DESC RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW),
	COUNT(*) OVER (),
	toFloat64(RANK() OVER (PARTITION BY p.Value1, c_1.ChildID ORDER BY p.Value1, c_1.ChildID)) + Sqrt(toFloat64(DENSE_RANK() OVER (PARTITION BY p.Value1, c_1.ChildID ORDER BY p.Value1))) + COUNT(p.ParentID) OVER (PARTITION BY p.Value1 ORDER BY p.Value1 RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) + COUNT(*) OVER ()
FROM
	Parent p
