...
FROM
	[Employees] [e]
		LEFT JOIN [Employees] [a_ReportsToNavigation] ON [e].[ReportsTo] = [a_ReportsToNavigation].[EmployeeID] AND [e].[ReportsTo] IS NOT NULL AND ([a_ReportsToNavigation].[IsDeleted] = 0 OR [a_ReportsToNavigation].[IsDeleted] = 0)
WHERE
