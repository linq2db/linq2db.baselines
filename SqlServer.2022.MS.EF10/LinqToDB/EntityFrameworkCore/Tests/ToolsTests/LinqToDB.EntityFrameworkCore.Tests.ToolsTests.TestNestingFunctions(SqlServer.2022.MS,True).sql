Parameters:
@ef_filter__p0='?' (DbType = Boolean), @ef_filter__p2='?' (DbType = Boolean)
...
CROSS JOIN (
    ...
    FROM [Products] AS [p0]
    WHERE (@ef_filter__p0 = CAST(1 AS bit) OR [p0].[IsDeleted] = CAST(0 AS bit) OR [p0].[IsDeleted] = CAST(0 AS bit)) AND EXISTS (
        SELECT 1
        FROM [Order Details] AS [o0]
        WHERE (@ef_filter__p2 = CAST(1 AS bit) OR [o0].[IsDeleted] = CAST(0 AS bit) OR [o0].[IsDeleted] = CAST(0 AS bit)) AND [p0].[ProductID] = [o0].[ProductID])
) AS [p1]
WHERE (@ef_filter__p0 = CAST(1 AS bit) OR [p].[IsDeleted] = CAST(0 AS bit) OR [p].[IsDeleted] = CAST(0 AS bit)) AND EXISTS (
    SELECT 1
    FROM [Order Details] AS [o]
    WHERE (@ef_filter__p2 = CAST(1 AS bit) OR [o].[IsDeleted] = CAST(0 AS bit) OR [o].[IsDeleted] = CAST(0 AS bit)) AND [p].[ProductID] = [o].[ProductID]) AND [p].[ProductID] = [p1].[ProductID]
