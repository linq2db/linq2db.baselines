...
IF (OBJECT_ID(N'Table7') IS NOT NULL)
	DROP TABLE [Table7]

BeforeExecute
-- Sybase.Managed Sybase

IF (OBJECT_ID(N'Table7') IS NULL)
	EXECUTE('
		CREATE TABLE [Table7]
		(
			[Field4] Int           NOT NULL,
...
FROM
	[Table2] [t1]
		...
		ON [a_Ref1].[Field3] = [t2].[Field3] AND ([a_Ref5].[Field8] = [a_Ref5_1].[Field8] AND [a_Ref5].[Field8] IS NOT NULL AND [a_Ref5_1].[Field8] IS NOT NULL OR [a_Ref5].[Field8] IS NULL AND [a_Ref5_1].[Field8] IS NULL)
