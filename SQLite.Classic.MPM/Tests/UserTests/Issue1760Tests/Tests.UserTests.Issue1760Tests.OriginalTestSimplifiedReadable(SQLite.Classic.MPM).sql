...
FROM
	[table1] [g_2]
		LEFT JOIN [table2] [bt1] ON [g_2].[c_tb1l_Id] = [bt1].[id] AND [g_2].[c_tb1l_Id] IS NOT NULL
		LEFT JOIN (
			SELECT
				[ctb].[Key_1] as [Id],
				[ctb].[maxCol],
			...
			FROM
				(
					...
					LEFT JOIN [table3] [btbl] ON [btbl].[col] = [tbl3].[col] AND [btbl].[col] IS NOT NULL AND [tbl3].[col] IS NOT NULL OR [btbl].[col] IS NULL AND [tbl3].[col] IS NULL
		) [ctb_1] ON [bt1].[col3] = [ctb_1].[Id_2] AND [bt1].[col3] IS NOT NULL AND [ctb_1].[Id_2] IS NOT NULL OR [bt1].[col3] IS NULL AND [ctb_1].[Id_2] IS NULL
