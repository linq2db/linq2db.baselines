Parameters:
@ef_filter__p2='?' (DbType = Boolean), @ef_filter__p4='?' (DbType = Boolean), @ef_filter__p8='?' (DbType = Boolean), @ef_filter__p6='?' (DbType = Boolean), @ef_filter__p0='?' (DbType = Boolean)
...
LEFT JOIN (
    ...
    FROM "Employees" AS e
    WHERE @ef_filter__p2 OR NOT (e."IsDeleted") OR NOT (e."IsDeleted")
) AS e0 ON o."EmployeeID" = e0."EmployeeID"
LEFT JOIN (
    SELECT e1."EmployeeID", e1."TerritoryID", e1."IsDeleted"
    FROM "EmployeeTerritories" AS e1
    WHERE @ef_filter__p4 OR NOT (e1."IsDeleted") OR NOT (e1."IsDeleted")
...
LEFT JOIN (
    ...
    INNER JOIN (
        ...
        FROM "Products" AS p
        WHERE @ef_filter__p8 OR NOT (p."IsDeleted") OR NOT (p."IsDeleted")
    ) AS p0 ON o0."ProductID" = p0."ProductID"
    WHERE @ef_filter__p6 OR NOT (o0."IsDeleted") OR NOT (o0."IsDeleted")
) AS s ON o."OrderID" = s."OrderID"
WHERE @ef_filter__p0 OR NOT (o."IsDeleted") OR NOT (o."IsDeleted")
