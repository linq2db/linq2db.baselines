...
FROM
	`Issue3791Table` `t1`
		LEFT JOIN `Issue3791GuidTable` `a_Association` ON (`t1`.`OtherId` = Cast(`a_Association`.`Id` as CHAR(255)) OR `t1`.`OtherId` IS NULL AND Cast(`a_Association`.`Id` as CHAR(255)) IS NULL)
