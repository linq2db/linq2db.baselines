Parameters:
@ef_filter__p5='?' (DbType = Boolean), @ef_filter__p1='?' (DbType = Boolean), @ef_filter__p7='?' (DbType = Boolean), @ef_filter__p3='?' (DbType = Boolean)
...
LEFT JOIN (
    ...
    INNER JOIN (
        ...
        FROM [Products] AS [p0]
        WHERE @ef_filter__p1 = CAST(1 AS bit) OR [p0].[IsDeleted] = CAST(0 AS bit) OR [p0].[IsDeleted] = CAST(0 AS bit)
    ) AS [p1] ON [o].[ProductID] = [p1].[ProductID]
    LEFT JOIN (
        SELECT [s].[SupplierID]
        FROM [Suppliers] AS [s]
        WHERE @ef_filter__p7 = CAST(1 AS bit) OR [s].[IsDeleted] = CAST(0 AS bit) OR [s].[IsDeleted] = CAST(0 AS bit)
    ...
    LEFT JOIN (
        ...
        FROM [Products] AS [p2]
        WHERE @ef_filter__p1 = CAST(1 AS bit) OR [p2].[IsDeleted] = CAST(0 AS bit) OR [p2].[IsDeleted] = CAST(0 AS bit)
