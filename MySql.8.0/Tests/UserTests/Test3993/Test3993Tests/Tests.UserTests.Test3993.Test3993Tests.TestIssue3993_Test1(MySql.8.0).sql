...
FROM
	`Common_Topology_Locations` `x`
WHERE
	Date_Add(`x`.`StartDateTime2`, Interval (@Value * 1000) Microsecond) > '2025-08-31 18:07:32.556'
...
FROM
	`Common_Topology_Locations` `x`
WHERE
	Date_Add(`x`.`StartDateTime2`, Interval (@Value * 1000) Microsecond) > '2025-08-31 18:07:32.561'
...
FROM
	`Common_Topology_Locations` `x`
WHERE
	CAST((TIMESTAMPDIFF(MICROSECOND, `x`.`StartDateTime`, `x`.`EndDateTime`) * 10) / 36000000000 AS DOUBLE) < 5
...
FROM
	`Common_Topology_Locations` `x`
WHERE
	CAST((TIMESTAMPDIFF(MICROSECOND, `x`.`StartDateTime`, `x`.`EndDateTime`) * 10) / 36000000000 AS DOUBLE) < 2
