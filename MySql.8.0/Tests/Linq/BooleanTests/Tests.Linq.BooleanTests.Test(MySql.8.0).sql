BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80
DECLARE @True UByte -- Boolean
SET     @True = 1

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`Boolean` = @True

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80
DECLARE @False UByte -- Boolean
SET     @False = 0

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`Boolean` = @False

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80
DECLARE @TrueN UByte -- Boolean
SET     @TrueN = 1

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`Boolean` = @TrueN

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80
DECLARE @FalseN UByte -- Boolean
SET     @FalseN = 0

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`Boolean` = @FalseN

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	1 = 0

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80
DECLARE @True UByte -- Boolean
SET     @True = 1

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`BooleanN` = @True

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80
DECLARE @False UByte -- Boolean
SET     @False = 0

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`BooleanN` = @False

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80
DECLARE @TrueN UByte -- Boolean
SET     @TrueN = 1

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`BooleanN` = @TrueN

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80
DECLARE @FalseN UByte -- Boolean
SET     @FalseN = 0

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`BooleanN` = @FalseN

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`BooleanN` IS NULL

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80
DECLARE @True UByte -- Boolean
SET     @True = 1

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`Boolean` <> @True

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80
DECLARE @False UByte -- Boolean
SET     @False = 0

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`Boolean` <> @False

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80
DECLARE @TrueN UByte -- Boolean
SET     @TrueN = 1

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`Boolean` <> @TrueN

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80
DECLARE @FalseN UByte -- Boolean
SET     @FalseN = 0

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`Boolean` <> @FalseN

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80
DECLARE @True UByte -- Boolean
SET     @True = 1

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`BooleanN` <> @True OR `r`.`BooleanN` IS NULL

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80
DECLARE @False UByte -- Boolean
SET     @False = 0

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`BooleanN` <> @False OR `r`.`BooleanN` IS NULL

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80
DECLARE @TrueN UByte -- Boolean
SET     @TrueN = 1

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`BooleanN` <> @TrueN OR `r`.`BooleanN` IS NULL

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80
DECLARE @FalseN UByte -- Boolean
SET     @FalseN = 0

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`BooleanN` <> @FalseN OR `r`.`BooleanN` IS NULL

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`BooleanN` IS NOT NULL

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`g_1`.`Id`,
	COUNT(CASE
		WHEN `g_1`.`Boolean` THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Boolean` = 1 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`BooleanN` = 1 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Boolean` = 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`BooleanN` = 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Boolean` = 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`BooleanN` = 0 OR `g_1`.`BooleanN` IS NULL THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Boolean` = 1 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`BooleanN` = 1 OR `g_1`.`BooleanN` IS NULL THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Int32` = 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Int32N` = 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Decimal` = CAST(0 AS DECIMAL(1)) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`DecimalN` = CAST(0 AS DECIMAL(1)) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Double` = CAST(0 AS DOUBLE) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`DoubleN` = CAST(0 AS DOUBLE) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Int32` <> 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Int32N` <> 0 OR `g_1`.`Int32N` IS NULL THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Decimal` <> CAST(0 AS DECIMAL(1)) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`DecimalN` <> CAST(0 AS DECIMAL(1)) OR `g_1`.`DecimalN` IS NULL
			THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Double` <> CAST(0 AS DOUBLE) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`DoubleN` <> CAST(0 AS DOUBLE) OR `g_1`.`DoubleN` IS NULL
			THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Int32` > 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Int32N` > 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Decimal` > CAST(0 AS DECIMAL(1)) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`DecimalN` > CAST(0 AS DECIMAL(1)) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Double` > CAST(0 AS DOUBLE) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`DoubleN` > CAST(0 AS DOUBLE) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Int32` < 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Int32N` < 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Decimal` < CAST(0 AS DECIMAL(1)) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`DecimalN` < CAST(0 AS DECIMAL(1)) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Double` < CAST(0 AS DOUBLE) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`DoubleN` < CAST(0 AS DOUBLE) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Int32` >= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Int32N` >= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Decimal` >= CAST(0 AS DECIMAL(1)) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`DecimalN` >= CAST(0 AS DECIMAL(1)) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Double` >= CAST(0 AS DOUBLE) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`DoubleN` >= CAST(0 AS DOUBLE) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Int32` <= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Int32N` <= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Decimal` <= CAST(0 AS DECIMAL(1)) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`DecimalN` <= CAST(0 AS DECIMAL(1)) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Double` <= CAST(0 AS DOUBLE) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`DoubleN` <= CAST(0 AS DOUBLE) THEN 1
		ELSE NULL
	END)
FROM
	`BooleanTable` `g_1`
GROUP BY
	`g_1`.`Id`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`r`.`Id`,
	`r`.`Int32` = 0,
	CASE
		WHEN `r`.`Int32N` = 0 THEN 1
		ELSE 0
	END,
	`r`.`Decimal` = CAST(0 AS DECIMAL(1)),
	CASE
		WHEN `r`.`DecimalN` = CAST(0 AS DECIMAL(1)) THEN 1
		ELSE 0
	END,
	`r`.`Double` = CAST(0 AS DOUBLE),
	CASE
		WHEN `r`.`DoubleN` = CAST(0 AS DOUBLE) THEN 1
		ELSE 0
	END,
	Coalesce(`r`.`Int32N`, `r`.`Int32`) = 0,
	Coalesce(`r`.`DecimalN`, `r`.`Decimal`) = CAST(0 AS DECIMAL(1)),
	Coalesce(`r`.`DoubleN`, `r`.`Double`) = CAST(0 AS DOUBLE),
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`Int32N`
			ELSE `r`.`Int32`
		END = 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`Int32N`
			ELSE `r`.`Int32`
		END = 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`DecimalN`
			ELSE `r`.`Decimal`
		END = CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`DecimalN`
			ELSE `r`.`Decimal`
		END = CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`DoubleN`
			ELSE `r`.`Double`
		END = CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`DoubleN`
			ELSE `r`.`Double`
		END = CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	`r`.`Int32` > 0,
	CASE
		WHEN `r`.`Int32N` > 0 THEN 1
		ELSE 0
	END,
	`r`.`Decimal` > CAST(0 AS DECIMAL(1)),
	CASE
		WHEN `r`.`DecimalN` > CAST(0 AS DECIMAL(1)) THEN 1
		ELSE 0
	END,
	`r`.`Double` > CAST(0 AS DOUBLE),
	CASE
		WHEN `r`.`DoubleN` > CAST(0 AS DOUBLE) THEN 1
		ELSE 0
	END,
	Coalesce(`r`.`Int32N`, `r`.`Int32`) > 0,
	Coalesce(`r`.`DecimalN`, `r`.`Decimal`) > CAST(0 AS DECIMAL(1)),
	Coalesce(`r`.`DoubleN`, `r`.`Double`) > CAST(0 AS DOUBLE),
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`Int32N`
			ELSE `r`.`Int32`
		END > 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`Int32N`
			ELSE `r`.`Int32`
		END > 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`DecimalN`
			ELSE `r`.`Decimal`
		END > CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`DecimalN`
			ELSE `r`.`Decimal`
		END > CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`DoubleN`
			ELSE `r`.`Double`
		END > CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`DoubleN`
			ELSE `r`.`Double`
		END > CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	`r`.`Int32` >= 0,
	CASE
		WHEN `r`.`Int32N` >= 0 THEN 1
		ELSE 0
	END,
	`r`.`Decimal` >= CAST(0 AS DECIMAL(1)),
	CASE
		WHEN `r`.`DecimalN` >= CAST(0 AS DECIMAL(1)) THEN 1
		ELSE 0
	END,
	`r`.`Double` >= CAST(0 AS DOUBLE),
	CASE
		WHEN `r`.`DoubleN` >= CAST(0 AS DOUBLE) THEN 1
		ELSE 0
	END,
	Coalesce(`r`.`Int32N`, `r`.`Int32`) >= 0,
	Coalesce(`r`.`DecimalN`, `r`.`Decimal`) >= CAST(0 AS DECIMAL(1)),
	Coalesce(`r`.`DoubleN`, `r`.`Double`) >= CAST(0 AS DOUBLE),
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`Int32N`
			ELSE `r`.`Int32`
		END >= 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`Int32N`
			ELSE `r`.`Int32`
		END >= 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`DecimalN`
			ELSE `r`.`Decimal`
		END >= CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`DecimalN`
			ELSE `r`.`Decimal`
		END >= CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`DoubleN`
			ELSE `r`.`Double`
		END >= CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`DoubleN`
			ELSE `r`.`Double`
		END >= CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	`r`.`Int32` < 0,
	CASE
		WHEN `r`.`Int32N` < 0 THEN 1
		ELSE 0
	END,
	`r`.`Decimal` < CAST(0 AS DECIMAL(1)),
	CASE
		WHEN `r`.`DecimalN` < CAST(0 AS DECIMAL(1)) THEN 1
		ELSE 0
	END,
	`r`.`Double` < CAST(0 AS DOUBLE),
	CASE
		WHEN `r`.`DoubleN` < CAST(0 AS DOUBLE) THEN 1
		ELSE 0
	END,
	Coalesce(`r`.`Int32N`, `r`.`Int32`) < 0,
	Coalesce(`r`.`DecimalN`, `r`.`Decimal`) < CAST(0 AS DECIMAL(1)),
	Coalesce(`r`.`DoubleN`, `r`.`Double`) < CAST(0 AS DOUBLE),
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`Int32N`
			ELSE `r`.`Int32`
		END < 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`Int32N`
			ELSE `r`.`Int32`
		END < 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`DecimalN`
			ELSE `r`.`Decimal`
		END < CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`DecimalN`
			ELSE `r`.`Decimal`
		END < CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`DoubleN`
			ELSE `r`.`Double`
		END < CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`DoubleN`
			ELSE `r`.`Double`
		END < CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	`r`.`Int32` <= 0,
	CASE
		WHEN `r`.`Int32N` <= 0 THEN 1
		ELSE 0
	END,
	`r`.`Decimal` <= CAST(0 AS DECIMAL(1)),
	CASE
		WHEN `r`.`DecimalN` <= CAST(0 AS DECIMAL(1)) THEN 1
		ELSE 0
	END,
	`r`.`Double` <= CAST(0 AS DOUBLE),
	CASE
		WHEN `r`.`DoubleN` <= CAST(0 AS DOUBLE) THEN 1
		ELSE 0
	END,
	Coalesce(`r`.`Int32N`, `r`.`Int32`) <= 0,
	Coalesce(`r`.`DecimalN`, `r`.`Decimal`) <= CAST(0 AS DECIMAL(1)),
	Coalesce(`r`.`DoubleN`, `r`.`Double`) <= CAST(0 AS DOUBLE),
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`Int32N`
			ELSE `r`.`Int32`
		END <= 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`Int32N`
			ELSE `r`.`Int32`
		END <= 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`DecimalN`
			ELSE `r`.`Decimal`
		END <= CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`DecimalN`
			ELSE `r`.`Decimal`
		END <= CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`DoubleN`
			ELSE `r`.`Double`
		END <= CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`DoubleN`
			ELSE `r`.`Double`
		END <= CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END
FROM
	`BooleanTable` `r`
UNION ALL
SELECT
	`r_1`.`Id`,
	`r_1`.`Int32` = 0,
	CASE
		WHEN `r_1`.`Int32N` = 0 THEN 1
		ELSE 0
	END,
	`r_1`.`Decimal` = CAST(0 AS DECIMAL(1)),
	CASE
		WHEN `r_1`.`DecimalN` = CAST(0 AS DECIMAL(1)) THEN 1
		ELSE 0
	END,
	`r_1`.`Double` = CAST(0 AS DOUBLE),
	CASE
		WHEN `r_1`.`DoubleN` = CAST(0 AS DOUBLE) THEN 1
		ELSE 0
	END,
	Coalesce(`r_1`.`Int32N`, `r_1`.`Int32`) = 0,
	Coalesce(`r_1`.`DecimalN`, `r_1`.`Decimal`) = CAST(0 AS DECIMAL(1)),
	Coalesce(`r_1`.`DoubleN`, `r_1`.`Double`) = CAST(0 AS DOUBLE),
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`Int32N`
			ELSE `r_1`.`Int32`
		END = 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`Int32N`
			ELSE `r_1`.`Int32`
		END = 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`DecimalN`
			ELSE `r_1`.`Decimal`
		END = CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`DecimalN`
			ELSE `r_1`.`Decimal`
		END = CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`DoubleN`
			ELSE `r_1`.`Double`
		END = CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`DoubleN`
			ELSE `r_1`.`Double`
		END = CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	`r_1`.`Int32` > 0,
	CASE
		WHEN `r_1`.`Int32N` > 0 THEN 1
		ELSE 0
	END,
	`r_1`.`Decimal` > CAST(0 AS DECIMAL(1)),
	CASE
		WHEN `r_1`.`DecimalN` > CAST(0 AS DECIMAL(1)) THEN 1
		ELSE 0
	END,
	`r_1`.`Double` > CAST(0 AS DOUBLE),
	CASE
		WHEN `r_1`.`DoubleN` > CAST(0 AS DOUBLE) THEN 1
		ELSE 0
	END,
	Coalesce(`r_1`.`Int32N`, `r_1`.`Int32`) > 0,
	Coalesce(`r_1`.`DecimalN`, `r_1`.`Decimal`) > CAST(0 AS DECIMAL(1)),
	Coalesce(`r_1`.`DoubleN`, `r_1`.`Double`) > CAST(0 AS DOUBLE),
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`Int32N`
			ELSE `r_1`.`Int32`
		END > 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`Int32N`
			ELSE `r_1`.`Int32`
		END > 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`DecimalN`
			ELSE `r_1`.`Decimal`
		END > CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`DecimalN`
			ELSE `r_1`.`Decimal`
		END > CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`DoubleN`
			ELSE `r_1`.`Double`
		END > CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`DoubleN`
			ELSE `r_1`.`Double`
		END > CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	`r_1`.`Int32` >= 0,
	CASE
		WHEN `r_1`.`Int32N` >= 0 THEN 1
		ELSE 0
	END,
	`r_1`.`Decimal` >= CAST(0 AS DECIMAL(1)),
	CASE
		WHEN `r_1`.`DecimalN` >= CAST(0 AS DECIMAL(1)) THEN 1
		ELSE 0
	END,
	`r_1`.`Double` >= CAST(0 AS DOUBLE),
	CASE
		WHEN `r_1`.`DoubleN` >= CAST(0 AS DOUBLE) THEN 1
		ELSE 0
	END,
	Coalesce(`r_1`.`Int32N`, `r_1`.`Int32`) >= 0,
	Coalesce(`r_1`.`DecimalN`, `r_1`.`Decimal`) >= CAST(0 AS DECIMAL(1)),
	Coalesce(`r_1`.`DoubleN`, `r_1`.`Double`) >= CAST(0 AS DOUBLE),
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`Int32N`
			ELSE `r_1`.`Int32`
		END >= 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`Int32N`
			ELSE `r_1`.`Int32`
		END >= 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`DecimalN`
			ELSE `r_1`.`Decimal`
		END >= CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`DecimalN`
			ELSE `r_1`.`Decimal`
		END >= CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`DoubleN`
			ELSE `r_1`.`Double`
		END >= CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`DoubleN`
			ELSE `r_1`.`Double`
		END >= CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	`r_1`.`Int32` < 0,
	CASE
		WHEN `r_1`.`Int32N` < 0 THEN 1
		ELSE 0
	END,
	`r_1`.`Decimal` < CAST(0 AS DECIMAL(1)),
	CASE
		WHEN `r_1`.`DecimalN` < CAST(0 AS DECIMAL(1)) THEN 1
		ELSE 0
	END,
	`r_1`.`Double` < CAST(0 AS DOUBLE),
	CASE
		WHEN `r_1`.`DoubleN` < CAST(0 AS DOUBLE) THEN 1
		ELSE 0
	END,
	Coalesce(`r_1`.`Int32N`, `r_1`.`Int32`) < 0,
	Coalesce(`r_1`.`DecimalN`, `r_1`.`Decimal`) < CAST(0 AS DECIMAL(1)),
	Coalesce(`r_1`.`DoubleN`, `r_1`.`Double`) < CAST(0 AS DOUBLE),
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`Int32N`
			ELSE `r_1`.`Int32`
		END < 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`Int32N`
			ELSE `r_1`.`Int32`
		END < 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`DecimalN`
			ELSE `r_1`.`Decimal`
		END < CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`DecimalN`
			ELSE `r_1`.`Decimal`
		END < CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`DoubleN`
			ELSE `r_1`.`Double`
		END < CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`DoubleN`
			ELSE `r_1`.`Double`
		END < CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	`r_1`.`Int32` <= 0,
	CASE
		WHEN `r_1`.`Int32N` <= 0 THEN 1
		ELSE 0
	END,
	`r_1`.`Decimal` <= CAST(0 AS DECIMAL(1)),
	CASE
		WHEN `r_1`.`DecimalN` <= CAST(0 AS DECIMAL(1)) THEN 1
		ELSE 0
	END,
	`r_1`.`Double` <= CAST(0 AS DOUBLE),
	CASE
		WHEN `r_1`.`DoubleN` <= CAST(0 AS DOUBLE) THEN 1
		ELSE 0
	END,
	Coalesce(`r_1`.`Int32N`, `r_1`.`Int32`) <= 0,
	Coalesce(`r_1`.`DecimalN`, `r_1`.`Decimal`) <= CAST(0 AS DECIMAL(1)),
	Coalesce(`r_1`.`DoubleN`, `r_1`.`Double`) <= CAST(0 AS DOUBLE),
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`Int32N`
			ELSE `r_1`.`Int32`
		END <= 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`Int32N`
			ELSE `r_1`.`Int32`
		END <= 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`DecimalN`
			ELSE `r_1`.`Decimal`
		END <= CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`DecimalN`
			ELSE `r_1`.`Decimal`
		END <= CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`DoubleN`
			ELSE `r_1`.`Double`
		END <= CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`DoubleN`
			ELSE `r_1`.`Double`
		END <= CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END
FROM
	`BooleanTable` `r_1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`Boolean` = 1

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`Boolean` = 0

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`Boolean` = 1

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`Boolean` = 0

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	1 = 0

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`BooleanN` = 1

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`BooleanN` = 0

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`BooleanN` = 1

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`BooleanN` = 0

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`BooleanN` IS NULL

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`Boolean` <> 1

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`Boolean` <> 0

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`Boolean` <> 1

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`Boolean` <> 0

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`BooleanN` <> 1 OR `r`.`BooleanN` IS NULL

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`BooleanN` <> 0 OR `r`.`BooleanN` IS NULL

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`BooleanN` <> 1 OR `r`.`BooleanN` IS NULL

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`BooleanN` <> 0 OR `r`.`BooleanN` IS NULL

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`r`.`Id`,
	`r`.`Boolean`,
	`r`.`BooleanN`,
	`r`.`Int32`,
	`r`.`Int32N`,
	`r`.`Decimal`,
	`r`.`DecimalN`,
	`r`.`Double`,
	`r`.`DoubleN`
FROM
	`BooleanTable` `r`
WHERE
	`r`.`BooleanN` IS NOT NULL

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`g_1`.`Id`,
	COUNT(CASE
		WHEN `g_1`.`Boolean` THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Boolean` = 1 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`BooleanN` = 1 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Boolean` = 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`BooleanN` = 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Boolean` = 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`BooleanN` = 0 OR `g_1`.`BooleanN` IS NULL THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Boolean` = 1 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`BooleanN` = 1 OR `g_1`.`BooleanN` IS NULL THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Int32` = 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Int32N` = 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Decimal` = CAST(0 AS DECIMAL(1)) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`DecimalN` = CAST(0 AS DECIMAL(1)) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Double` = CAST(0 AS DOUBLE) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`DoubleN` = CAST(0 AS DOUBLE) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Int32` <> 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Int32N` <> 0 OR `g_1`.`Int32N` IS NULL THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Decimal` <> CAST(0 AS DECIMAL(1)) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`DecimalN` <> CAST(0 AS DECIMAL(1)) OR `g_1`.`DecimalN` IS NULL
			THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Double` <> CAST(0 AS DOUBLE) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`DoubleN` <> CAST(0 AS DOUBLE) OR `g_1`.`DoubleN` IS NULL
			THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Int32` > 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Int32N` > 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Decimal` > CAST(0 AS DECIMAL(1)) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`DecimalN` > CAST(0 AS DECIMAL(1)) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Double` > CAST(0 AS DOUBLE) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`DoubleN` > CAST(0 AS DOUBLE) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Int32` < 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Int32N` < 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Decimal` < CAST(0 AS DECIMAL(1)) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`DecimalN` < CAST(0 AS DECIMAL(1)) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Double` < CAST(0 AS DOUBLE) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`DoubleN` < CAST(0 AS DOUBLE) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Int32` >= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Int32N` >= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Decimal` >= CAST(0 AS DECIMAL(1)) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`DecimalN` >= CAST(0 AS DECIMAL(1)) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Double` >= CAST(0 AS DOUBLE) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`DoubleN` >= CAST(0 AS DOUBLE) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Int32` <= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Int32N` <= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Decimal` <= CAST(0 AS DECIMAL(1)) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`DecimalN` <= CAST(0 AS DECIMAL(1)) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`Double` <= CAST(0 AS DOUBLE) THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN `g_1`.`DoubleN` <= CAST(0 AS DOUBLE) THEN 1
		ELSE NULL
	END)
FROM
	`BooleanTable` `g_1`
GROUP BY
	`g_1`.`Id`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`r`.`Id`,
	`r`.`Int32` = 0,
	CASE
		WHEN `r`.`Int32N` = 0 THEN 1
		ELSE 0
	END,
	`r`.`Decimal` = CAST(0 AS DECIMAL(1)),
	CASE
		WHEN `r`.`DecimalN` = CAST(0 AS DECIMAL(1)) THEN 1
		ELSE 0
	END,
	`r`.`Double` = CAST(0 AS DOUBLE),
	CASE
		WHEN `r`.`DoubleN` = CAST(0 AS DOUBLE) THEN 1
		ELSE 0
	END,
	Coalesce(`r`.`Int32N`, `r`.`Int32`) = 0,
	Coalesce(`r`.`DecimalN`, `r`.`Decimal`) = CAST(0 AS DECIMAL(1)),
	Coalesce(`r`.`DoubleN`, `r`.`Double`) = CAST(0 AS DOUBLE),
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`Int32N`
			ELSE `r`.`Int32`
		END = 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`Int32N`
			ELSE `r`.`Int32`
		END = 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`DecimalN`
			ELSE `r`.`Decimal`
		END = CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`DecimalN`
			ELSE `r`.`Decimal`
		END = CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`DoubleN`
			ELSE `r`.`Double`
		END = CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`DoubleN`
			ELSE `r`.`Double`
		END = CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	`r`.`Int32` > 0,
	CASE
		WHEN `r`.`Int32N` > 0 THEN 1
		ELSE 0
	END,
	`r`.`Decimal` > CAST(0 AS DECIMAL(1)),
	CASE
		WHEN `r`.`DecimalN` > CAST(0 AS DECIMAL(1)) THEN 1
		ELSE 0
	END,
	`r`.`Double` > CAST(0 AS DOUBLE),
	CASE
		WHEN `r`.`DoubleN` > CAST(0 AS DOUBLE) THEN 1
		ELSE 0
	END,
	Coalesce(`r`.`Int32N`, `r`.`Int32`) > 0,
	Coalesce(`r`.`DecimalN`, `r`.`Decimal`) > CAST(0 AS DECIMAL(1)),
	Coalesce(`r`.`DoubleN`, `r`.`Double`) > CAST(0 AS DOUBLE),
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`Int32N`
			ELSE `r`.`Int32`
		END > 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`Int32N`
			ELSE `r`.`Int32`
		END > 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`DecimalN`
			ELSE `r`.`Decimal`
		END > CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`DecimalN`
			ELSE `r`.`Decimal`
		END > CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`DoubleN`
			ELSE `r`.`Double`
		END > CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`DoubleN`
			ELSE `r`.`Double`
		END > CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	`r`.`Int32` >= 0,
	CASE
		WHEN `r`.`Int32N` >= 0 THEN 1
		ELSE 0
	END,
	`r`.`Decimal` >= CAST(0 AS DECIMAL(1)),
	CASE
		WHEN `r`.`DecimalN` >= CAST(0 AS DECIMAL(1)) THEN 1
		ELSE 0
	END,
	`r`.`Double` >= CAST(0 AS DOUBLE),
	CASE
		WHEN `r`.`DoubleN` >= CAST(0 AS DOUBLE) THEN 1
		ELSE 0
	END,
	Coalesce(`r`.`Int32N`, `r`.`Int32`) >= 0,
	Coalesce(`r`.`DecimalN`, `r`.`Decimal`) >= CAST(0 AS DECIMAL(1)),
	Coalesce(`r`.`DoubleN`, `r`.`Double`) >= CAST(0 AS DOUBLE),
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`Int32N`
			ELSE `r`.`Int32`
		END >= 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`Int32N`
			ELSE `r`.`Int32`
		END >= 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`DecimalN`
			ELSE `r`.`Decimal`
		END >= CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`DecimalN`
			ELSE `r`.`Decimal`
		END >= CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`DoubleN`
			ELSE `r`.`Double`
		END >= CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`DoubleN`
			ELSE `r`.`Double`
		END >= CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	`r`.`Int32` < 0,
	CASE
		WHEN `r`.`Int32N` < 0 THEN 1
		ELSE 0
	END,
	`r`.`Decimal` < CAST(0 AS DECIMAL(1)),
	CASE
		WHEN `r`.`DecimalN` < CAST(0 AS DECIMAL(1)) THEN 1
		ELSE 0
	END,
	`r`.`Double` < CAST(0 AS DOUBLE),
	CASE
		WHEN `r`.`DoubleN` < CAST(0 AS DOUBLE) THEN 1
		ELSE 0
	END,
	Coalesce(`r`.`Int32N`, `r`.`Int32`) < 0,
	Coalesce(`r`.`DecimalN`, `r`.`Decimal`) < CAST(0 AS DECIMAL(1)),
	Coalesce(`r`.`DoubleN`, `r`.`Double`) < CAST(0 AS DOUBLE),
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`Int32N`
			ELSE `r`.`Int32`
		END < 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`Int32N`
			ELSE `r`.`Int32`
		END < 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`DecimalN`
			ELSE `r`.`Decimal`
		END < CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`DecimalN`
			ELSE `r`.`Decimal`
		END < CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`DoubleN`
			ELSE `r`.`Double`
		END < CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`DoubleN`
			ELSE `r`.`Double`
		END < CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	`r`.`Int32` <= 0,
	CASE
		WHEN `r`.`Int32N` <= 0 THEN 1
		ELSE 0
	END,
	`r`.`Decimal` <= CAST(0 AS DECIMAL(1)),
	CASE
		WHEN `r`.`DecimalN` <= CAST(0 AS DECIMAL(1)) THEN 1
		ELSE 0
	END,
	`r`.`Double` <= CAST(0 AS DOUBLE),
	CASE
		WHEN `r`.`DoubleN` <= CAST(0 AS DOUBLE) THEN 1
		ELSE 0
	END,
	Coalesce(`r`.`Int32N`, `r`.`Int32`) <= 0,
	Coalesce(`r`.`DecimalN`, `r`.`Decimal`) <= CAST(0 AS DECIMAL(1)),
	Coalesce(`r`.`DoubleN`, `r`.`Double`) <= CAST(0 AS DOUBLE),
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`Int32N`
			ELSE `r`.`Int32`
		END <= 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`Int32N`
			ELSE `r`.`Int32`
		END <= 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`DecimalN`
			ELSE `r`.`Decimal`
		END <= CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`DecimalN`
			ELSE `r`.`Decimal`
		END <= CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`Boolean` THEN `r`.`DoubleN`
			ELSE `r`.`Double`
		END <= CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r`.`BooleanN` = 0 THEN `r`.`DoubleN`
			ELSE `r`.`Double`
		END <= CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END
FROM
	`BooleanTable` `r`
UNION ALL
SELECT
	`r_1`.`Id`,
	`r_1`.`Int32` = 0,
	CASE
		WHEN `r_1`.`Int32N` = 0 THEN 1
		ELSE 0
	END,
	`r_1`.`Decimal` = CAST(0 AS DECIMAL(1)),
	CASE
		WHEN `r_1`.`DecimalN` = CAST(0 AS DECIMAL(1)) THEN 1
		ELSE 0
	END,
	`r_1`.`Double` = CAST(0 AS DOUBLE),
	CASE
		WHEN `r_1`.`DoubleN` = CAST(0 AS DOUBLE) THEN 1
		ELSE 0
	END,
	Coalesce(`r_1`.`Int32N`, `r_1`.`Int32`) = 0,
	Coalesce(`r_1`.`DecimalN`, `r_1`.`Decimal`) = CAST(0 AS DECIMAL(1)),
	Coalesce(`r_1`.`DoubleN`, `r_1`.`Double`) = CAST(0 AS DOUBLE),
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`Int32N`
			ELSE `r_1`.`Int32`
		END = 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`Int32N`
			ELSE `r_1`.`Int32`
		END = 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`DecimalN`
			ELSE `r_1`.`Decimal`
		END = CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`DecimalN`
			ELSE `r_1`.`Decimal`
		END = CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`DoubleN`
			ELSE `r_1`.`Double`
		END = CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`DoubleN`
			ELSE `r_1`.`Double`
		END = CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	`r_1`.`Int32` > 0,
	CASE
		WHEN `r_1`.`Int32N` > 0 THEN 1
		ELSE 0
	END,
	`r_1`.`Decimal` > CAST(0 AS DECIMAL(1)),
	CASE
		WHEN `r_1`.`DecimalN` > CAST(0 AS DECIMAL(1)) THEN 1
		ELSE 0
	END,
	`r_1`.`Double` > CAST(0 AS DOUBLE),
	CASE
		WHEN `r_1`.`DoubleN` > CAST(0 AS DOUBLE) THEN 1
		ELSE 0
	END,
	Coalesce(`r_1`.`Int32N`, `r_1`.`Int32`) > 0,
	Coalesce(`r_1`.`DecimalN`, `r_1`.`Decimal`) > CAST(0 AS DECIMAL(1)),
	Coalesce(`r_1`.`DoubleN`, `r_1`.`Double`) > CAST(0 AS DOUBLE),
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`Int32N`
			ELSE `r_1`.`Int32`
		END > 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`Int32N`
			ELSE `r_1`.`Int32`
		END > 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`DecimalN`
			ELSE `r_1`.`Decimal`
		END > CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`DecimalN`
			ELSE `r_1`.`Decimal`
		END > CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`DoubleN`
			ELSE `r_1`.`Double`
		END > CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`DoubleN`
			ELSE `r_1`.`Double`
		END > CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	`r_1`.`Int32` >= 0,
	CASE
		WHEN `r_1`.`Int32N` >= 0 THEN 1
		ELSE 0
	END,
	`r_1`.`Decimal` >= CAST(0 AS DECIMAL(1)),
	CASE
		WHEN `r_1`.`DecimalN` >= CAST(0 AS DECIMAL(1)) THEN 1
		ELSE 0
	END,
	`r_1`.`Double` >= CAST(0 AS DOUBLE),
	CASE
		WHEN `r_1`.`DoubleN` >= CAST(0 AS DOUBLE) THEN 1
		ELSE 0
	END,
	Coalesce(`r_1`.`Int32N`, `r_1`.`Int32`) >= 0,
	Coalesce(`r_1`.`DecimalN`, `r_1`.`Decimal`) >= CAST(0 AS DECIMAL(1)),
	Coalesce(`r_1`.`DoubleN`, `r_1`.`Double`) >= CAST(0 AS DOUBLE),
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`Int32N`
			ELSE `r_1`.`Int32`
		END >= 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`Int32N`
			ELSE `r_1`.`Int32`
		END >= 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`DecimalN`
			ELSE `r_1`.`Decimal`
		END >= CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`DecimalN`
			ELSE `r_1`.`Decimal`
		END >= CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`DoubleN`
			ELSE `r_1`.`Double`
		END >= CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`DoubleN`
			ELSE `r_1`.`Double`
		END >= CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	`r_1`.`Int32` < 0,
	CASE
		WHEN `r_1`.`Int32N` < 0 THEN 1
		ELSE 0
	END,
	`r_1`.`Decimal` < CAST(0 AS DECIMAL(1)),
	CASE
		WHEN `r_1`.`DecimalN` < CAST(0 AS DECIMAL(1)) THEN 1
		ELSE 0
	END,
	`r_1`.`Double` < CAST(0 AS DOUBLE),
	CASE
		WHEN `r_1`.`DoubleN` < CAST(0 AS DOUBLE) THEN 1
		ELSE 0
	END,
	Coalesce(`r_1`.`Int32N`, `r_1`.`Int32`) < 0,
	Coalesce(`r_1`.`DecimalN`, `r_1`.`Decimal`) < CAST(0 AS DECIMAL(1)),
	Coalesce(`r_1`.`DoubleN`, `r_1`.`Double`) < CAST(0 AS DOUBLE),
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`Int32N`
			ELSE `r_1`.`Int32`
		END < 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`Int32N`
			ELSE `r_1`.`Int32`
		END < 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`DecimalN`
			ELSE `r_1`.`Decimal`
		END < CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`DecimalN`
			ELSE `r_1`.`Decimal`
		END < CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`DoubleN`
			ELSE `r_1`.`Double`
		END < CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`DoubleN`
			ELSE `r_1`.`Double`
		END < CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	`r_1`.`Int32` <= 0,
	CASE
		WHEN `r_1`.`Int32N` <= 0 THEN 1
		ELSE 0
	END,
	`r_1`.`Decimal` <= CAST(0 AS DECIMAL(1)),
	CASE
		WHEN `r_1`.`DecimalN` <= CAST(0 AS DECIMAL(1)) THEN 1
		ELSE 0
	END,
	`r_1`.`Double` <= CAST(0 AS DOUBLE),
	CASE
		WHEN `r_1`.`DoubleN` <= CAST(0 AS DOUBLE) THEN 1
		ELSE 0
	END,
	Coalesce(`r_1`.`Int32N`, `r_1`.`Int32`) <= 0,
	Coalesce(`r_1`.`DecimalN`, `r_1`.`Decimal`) <= CAST(0 AS DECIMAL(1)),
	Coalesce(`r_1`.`DoubleN`, `r_1`.`Double`) <= CAST(0 AS DOUBLE),
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`Int32N`
			ELSE `r_1`.`Int32`
		END <= 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`Int32N`
			ELSE `r_1`.`Int32`
		END <= 0
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`DecimalN`
			ELSE `r_1`.`Decimal`
		END <= CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`DecimalN`
			ELSE `r_1`.`Decimal`
		END <= CAST(0 AS DECIMAL(1))
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`Boolean` THEN `r_1`.`DoubleN`
			ELSE `r_1`.`Double`
		END <= CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END,
	CASE
		WHEN CASE
			WHEN `r_1`.`BooleanN` = 0 THEN `r_1`.`DoubleN`
			ELSE `r_1`.`Double`
		END <= CAST(0 AS DOUBLE)
			THEN 1
		ELSE 0
	END
FROM
	`BooleanTable` `r_1`

BeforeExecute
-- MySql.8.0 MySql.8.0.MySql.Data MySql80

SELECT
	`t1`.`Id`,
	`t1`.`Boolean`,
	`t1`.`BooleanN`,
	`t1`.`Int32`,
	`t1`.`Int32N`,
	`t1`.`Decimal`,
	`t1`.`DecimalN`,
	`t1`.`Double`,
	`t1`.`DoubleN`
FROM
	`BooleanTable` `t1`

