	...
	(
		...
		FROM
			(
				...
				LEFT JOIN [Suppliers] [a_Supplier] ON [a_Product].[SupplierID] = [a_Supplier].[SupplierID] AND [a_Product].[SupplierID] IS NOT NULL AND NOT [a_Supplier].[IsDeleted]
		WHERE
...
FROM
	[Products] [m_1]
		...
		LEFT JOIN [Suppliers] [a_Supplier] ON [m_1].[SupplierID] = [a_Supplier].[SupplierID] AND [m_1].[SupplierID] IS NOT NULL AND NOT [a_Supplier].[IsDeleted]
WHERE
