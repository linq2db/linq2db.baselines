...
LEFT JOIN (
    ...
    WHERE @__ef_filter__p_0 OR NOT ("e0"."IsDeleted")
) AS "t" ON "e"."ReportsTo" = "t"."EmployeeID"
WHERE @__ef_filter__p_0 OR NOT ("e"."IsDeleted")
...
FROM
	[Employees] [e]
		LEFT JOIN [Employees] [a_ReportsToNavigation] ON [e].[ReportsTo] = [a_ReportsToNavigation].[EmployeeID] AND [e].[ReportsTo] IS NOT NULL AND NOT [a_ReportsToNavigation].[IsDeleted]
WHERE
	NOT [e].[IsDeleted]
