...
FROM
	"Parent" p
		LEFT JOIN "Parent" a_ParentTest ON p."ParentID" = a_ParentTest."ParentID" AND (p."Value1" = a_ParentTest."Value1" AND p."Value1" IS NOT NULL AND a_ParentTest."Value1" IS NOT NULL OR p."Value1" IS NULL AND a_ParentTest."Value1" IS NULL)
