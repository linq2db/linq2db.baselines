...
(
	...
	WHERE
		`ncGroupMember`.`NC_GROUP_BO` = (CONCAT('NCGroupBO:', Coalesce(`ncCode`.`SITE`, ''), ',CATAN_AUTO')) OR `ncGroupMember`.`NC_GROUP_BO` = (CONCAT('NCGroupBO:', Coalesce(`ncCode`.`SITE`, ''), ',CATAN_MAN')) OR `ncGroupMember`.`NC_GROUP_BO` = (CONCAT('NCGroupBO:', Coalesce(`ncCode`.`SITE`, ''), ',CATAN_ALL'))
)
SELECT
	`item_1`.`NcCodeBo`,
	`item_1`.`NcCode`,
