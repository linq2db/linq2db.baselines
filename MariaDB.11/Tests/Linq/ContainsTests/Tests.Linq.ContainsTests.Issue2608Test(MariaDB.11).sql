...
FROM
	(
		SELECT
			CAST(`f`.`StringValue` AS SIGNED) as `StatusPhase`
		FROM
			`LinqDataTypes` `x`
				...
				INNER JOIN `LinqDataTypes` `r` ON (`u`.`IntValue` = `r`.`IntValue` OR `u`.`IntValue` IS NULL AND `r`.`IntValue` IS NULL) AND (`u`.`IntValue` = `r`.`IntValue` OR `u`.`IntValue` IS NULL AND `r`.`IntValue` IS NULL) AND `r`.`BoolValue` = 0
