...
FROM
	(
		SELECT
			...
			CAST(NULL AS SIGNED) as `Item1_1`,
			CAST(NULL AS CHAR(255)) as `Item2_1`,
			CAST(NULL AS SIGNED) as `Item3_1`
		FROM
			`CTE_2` `t3`
		UNION ALL
		SELECT
		...
		SELECT
			...
			`t5`.`Id` as `Item2`,
			`t5`.`field_1` as `Item3`,
			CAST(NULL AS SIGNED) as `Item4`,
			CAST(NULL AS CHAR(255)) as `Item5`,
			NULL as `Item6`,
			NULL as `Item7`,
			`t5`.`Id` as `Item1_1`,
			`t5`.`Name` as `Item2_1`,
