...
FROM
	[Table5] [t1]
		LEFT JOIN [Table5] [a_Table5Ref] ON [t1].[Field5] = [a_Table5Ref].[ProblematicalField] AND [t1].[Field5] IS NOT NULL
		INNER JOIN [Table4] [t2] ON [t1].[ProblematicalField] = [t2].[Field4]
		INNER JOIN [Table1] [a_Table1s] ON [t2].[Field3] = [a_Table1s].[Field3] AND [a_Table1s].[Field3] IS NOT NULL
		LEFT JOIN [Table4] [a_Table4Ref] ON [a_Table1s].[Field3] = [a_Table4Ref].[Field3] AND [a_Table1s].[Field3] IS NOT NULL
		LEFT JOIN [Table5] [a_Table5Ref_1] ON [a_Table4Ref].[Field4] = [a_Table5Ref_1].[ProblematicalField]
		LEFT JOIN [Table5] [a_Table5Ref_2] ON [a_Table5Ref_1].[Field5] = [a_Table5Ref_2].[ProblematicalField] AND [a_Table5Ref_1].[Field5] IS NOT NULL
		INNER JOIN [Table4] [t4] ON [a_Table5Ref_2].[ProblematicalField] = [t4].[Field4]
		INNER JOIN [Table1] [t6] ON [t4].[Field3] = [t6].[Field3] AND [t6].[Field3] IS NOT NULL
		INNER JOIN [Table4] [t7] ON [a_Table5Ref].[ProblematicalField] = [t7].[Field4]
		INNER JOIN [Table1] [a_Table1s_1] ON [t7].[Field3] = [a_Table1s_1].[Field3] AND [a_Table1s_1].[Field3] IS NOT NULL
