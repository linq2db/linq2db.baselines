...
LEFT JOIN (
    ...
    INNER JOIN (
        ...
        FROM "Products" AS "p"
        WHERE (@ef_filter__p5 OR "p"."ProductID" > 2) AND (@ef_filter__p5 OR NOT ("p"."Discontinued")) AND (@ef_filter__p1 OR NOT ("p"."IsDeleted") OR NOT ("p"."IsDeleted"))
    ) AS "p0" ON "o0"."ProductID" = "p0"."ProductID"
    LEFT JOIN (
        SELECT "o1"."OrderID", "o1"."ProductID", "o1"."Discount", "o1"."IsDeleted", "o1"."Quantity", "o1"."UnitPrice"
        FROM "Order Details" AS "o1"
        WHERE @ef_filter__p1 OR NOT ("o1"."IsDeleted") OR NOT ("o1"."IsDeleted")
...
FROM
	(
		...
		WHERE
			NOT [e].[IsDeleted] AND NOT [a_Product].[IsDeleted] AND
			NOT [d].[IsDeleted]
	) [m_1]
