...
(
	...
	FROM
		(
			SELECT
				'ActiveOnly'::NVarChar(255) as Label,
				c_1.Id
			FROM
				Company c_1
			UNION ALL
			SELECT
				'All'::NVarChar(255) as Label,
				c_2.Id
			FROM
				Company c_2
		) t1
...
FROM
	(
		SELECT
			...
			t4.field_1 as Item3,
			NVL(t4.Detail_Id, NULL) as Item4,
			NVL(t4.Detail_CompanyId, NULL) as Item5,
			t4.Detail_Name as Item6,
			NVL(t4.Detail_IsActive, NULL) as Item7,
			NULL::NVarChar(255) as Item1_2,
			NULL::Int as Item2_1,
			NULL::BOOLEAN as Item3_1,
			NULL::Int as Item4_1,
			NULL::NVarChar(255) as Item5_1
		FROM
			CTE_2 t4
		UNION ALL
		SELECT
