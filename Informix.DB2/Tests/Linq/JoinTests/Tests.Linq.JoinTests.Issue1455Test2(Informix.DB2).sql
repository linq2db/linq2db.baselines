...
FROM
	(
		...
		WHERE
			(nomin_2.DeliveryCounterParty LIKE '%C%' OR trade_2.CounterParty LIKE '%C%' OR al_group_1.AlertCode LIKE '%C%')
		GROUP BY
			al_group_1.AlertKey,
			al_group_1.AlertCode,
			al_group_1.CreationDate
		...
		LEFT JOIN (
			...
			WHERE
				(nomin_4.DeliveryCounterParty LIKE '%C%' OR trade_4.CounterParty LIKE '%C%' OR t1.AlertCode LIKE '%C%')
