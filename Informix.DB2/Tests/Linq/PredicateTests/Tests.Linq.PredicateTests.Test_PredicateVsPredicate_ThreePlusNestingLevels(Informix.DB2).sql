...
WHERE
	...
	END = CASE
		WHEN CASE
			WHEN (r.Value4 = r.Value5 OR r.Value4 IS NULL AND r.Value5 IS NULL) AND NOT (r.Value4 IS NULL AND r.Value5 IS NOT NULL) AND NOT (r.Value4 IS NOT NULL AND r.Value5 IS NULL)
				THEN 't'::BOOLEAN
			ELSE 'f'::BOOLEAN
		END = CASE
