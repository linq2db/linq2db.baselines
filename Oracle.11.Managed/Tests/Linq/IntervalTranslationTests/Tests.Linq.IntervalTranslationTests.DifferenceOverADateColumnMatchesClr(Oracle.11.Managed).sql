...
-- Oracle.11.Managed Oracle11
SELECT
	CAST(Floor(Extract(Day From (CAST(r."ClosedOn" AS timestamp) - CAST(r."OpenedOn" AS timestamp)))) AS Number(19)) * 864000000000 + CAST(Floor(Extract(Hour From (CAST(r."ClosedOn" AS timestamp) - CAST(r."OpenedOn" AS timestamp)))) AS Number(19)) * 36000000000 + CAST(Floor(Extract(Minute From (CAST(r."ClosedOn" AS timestamp) - CAST(r."OpenedOn" AS timestamp)))) AS Number(19)) * 600000000 + CAST(Floor(Round(Extract(Second From (CAST(r."ClosedOn" AS timestamp) - CAST(r."OpenedOn" AS timestamp))) * 10000000D)) AS Number(19))
FROM
	"CoarseEventRow" r
WHERE
	ROWNUM <= 2
