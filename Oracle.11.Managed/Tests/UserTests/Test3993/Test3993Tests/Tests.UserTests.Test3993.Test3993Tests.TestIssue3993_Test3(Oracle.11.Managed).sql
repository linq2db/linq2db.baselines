...
FROM
	"Common_Topology_Locations" t
WHERE
	EXTRACT(HOUR FROM (t."StartDateTime" + (-t."PreNotification" / 10000) * INTERVAL '0.001' SECOND)) = 13 AND
	ROWNUM <= 1
...
FROM
	"Common_Topology_Locations" t
WHERE
	EXTRACT(MINUTE FROM (t."StartDateTime" + (-t."PreNotification" / 10000) * INTERVAL '0.001' SECOND)) = 51 AND
	ROWNUM <= 1
...
WHERE
	t1."Second_1" >= 52 AND t1."Second_1" <= 54 AND ROWNUM <= 1
