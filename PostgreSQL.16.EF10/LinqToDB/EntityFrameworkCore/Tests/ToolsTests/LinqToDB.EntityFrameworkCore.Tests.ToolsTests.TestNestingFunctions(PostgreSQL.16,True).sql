...
CROSS JOIN (
    ...
    FROM "Products" AS p0
    WHERE (@ef_filter__p1 OR NOT (p0."IsDeleted") OR NOT (p0."IsDeleted")) AND EXISTS (
        SELECT 1
        FROM "Order Details" AS o0
        WHERE (@ef_filter__p3 OR NOT (o0."IsDeleted") OR NOT (o0."IsDeleted")) AND p0."ProductID" = o0."ProductID")
) AS p1
WHERE (@ef_filter__p1 OR NOT (p."IsDeleted") OR NOT (p."IsDeleted")) AND EXISTS (
    SELECT 1
    FROM "Order Details" AS o
    WHERE (@ef_filter__p3 OR NOT (o."IsDeleted") OR NOT (o."IsDeleted")) AND p."ProductID" = o."ProductID") AND p."ProductID" = p1."ProductID"
