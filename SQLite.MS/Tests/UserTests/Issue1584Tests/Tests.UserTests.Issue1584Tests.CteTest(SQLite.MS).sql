...
(
	...
	FROM
		[RateEntry] [rateEntry_1]
			LEFT JOIN [RateLines] [rateLine] ON [rateLine].[TL_TI] = [rateEntry_1].[TI_PK]
			LEFT JOIN [RateLineItem] [rateLineItem_1] ON [rateLineItem_1].[TM_TL] = [rateLine].[TL_PK]
	WHERE
		([rateEntry_1].[TI_RateEndDate] IS NULL OR strftime('%Y-%m-%d %H:%M:%f', [rateEntry_1].[TI_RateEndDate]) > strftime('%Y-%m-%d %H:%M:%f', CURRENT_TIMESTAMP)) AND
