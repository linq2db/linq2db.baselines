...
SELECT
	...
	Coalesce((
		SELECT
			GROUP_CONCAT(Coalesce([t2].[NullableDistinct], ''), ', ')
		FROM
			(
				SELECT DISTINCT
	...
	Coalesce((
		SELECT
			GROUP_CONCAT(Coalesce([t3].[NullableDistinctNotNullDistinct], ''), ', ')
		FROM
			(
				SELECT DISTINCT
	...
	Coalesce((
		SELECT
			GROUP_CONCAT(Coalesce([t4].[NullableDistinctNotNullDistinctOrdered], ''), ', ')
		FROM
			(
				SELECT DISTINCT
	...
	Coalesce((
		SELECT
			GROUP_CONCAT([t5].[NotNullableDistinct], ', ')
		FROM
			(
				SELECT DISTINCT
	...
	Coalesce((
		SELECT
			GROUP_CONCAT([t6].[NotNullableDistinctOrdered], ', ')
		FROM
			(
				SELECT DISTINCT
