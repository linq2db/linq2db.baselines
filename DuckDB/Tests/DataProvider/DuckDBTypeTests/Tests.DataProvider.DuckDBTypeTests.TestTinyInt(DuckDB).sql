...
FROM
	"TypeTable`2" r
WHERE
	r."Column" = CAST($value AS TINYINT) AND r.ColumnNullable IS NULL
...
(
	CAST($Id AS INTEGER),
	CAST($Column AS TINYINT),
	CAST($ColumnNullable AS TINYINT)
)
...
FROM
	"TypeTable`2" r
WHERE
	r."Column" = CAST($value AS TINYINT) AND r.ColumnNullable = CAST($nullableValue AS TINYINT)
...
FROM
	"TypeTable`2" r
WHERE
	r."Column" = '127'::FLOAT AND r.ColumnNullable = '-128'::FLOAT
...
FROM
	"TypeTable`2" r
WHERE
	r."Column" = '-128'::FLOAT AND r.ColumnNullable = '127'::FLOAT
