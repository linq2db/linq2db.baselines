...
FROM
	"TypeTable`2" r
WHERE
	r."Column" = CAST($value AS INTEGER) AND r.ColumnNullable IS NULL
...
(
	...
	CAST($ColumnNullable AS INTEGER)
)
...
FROM
	"TypeTable`2" r
WHERE
	r."Column" = CAST($value AS INTEGER) AND r.ColumnNullable = CAST($nullableValue AS INTEGER)
