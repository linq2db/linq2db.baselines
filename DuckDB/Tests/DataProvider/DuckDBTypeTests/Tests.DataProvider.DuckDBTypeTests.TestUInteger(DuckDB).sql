...
FROM
	"TypeTable`2" r
WHERE
	r."Column" = CAST($value AS UINTEGER) AND r.ColumnNullable IS NULL
...
(
	CAST($Id AS INTEGER),
	CAST($Column AS UINTEGER),
	CAST($ColumnNullable AS UINTEGER)
)
...
FROM
	"TypeTable`2" r
WHERE
	r."Column" = CAST($value AS UINTEGER) AND r.ColumnNullable = CAST($nullableValue AS UINTEGER)
