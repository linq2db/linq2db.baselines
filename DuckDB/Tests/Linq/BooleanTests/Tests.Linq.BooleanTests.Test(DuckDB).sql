-- DuckDB
DECLARE $True  -- Boolean
SET     $True = True

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r."Boolean" = CAST($True AS BOOLEAN)

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB
DECLARE $False  -- Boolean
SET     $False = False

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r."Boolean" = CAST($False AS BOOLEAN)

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB
DECLARE $TrueN  -- Boolean
SET     $TrueN = True

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r."Boolean" = CAST($TrueN AS BOOLEAN)

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB
DECLARE $FalseN  -- Boolean
SET     $FalseN = False

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r."Boolean" = CAST($FalseN AS BOOLEAN)

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	1 = 0

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB
DECLARE $True  -- Boolean
SET     $True = True

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r.BooleanN = CAST($True AS BOOLEAN)

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB
DECLARE $False  -- Boolean
SET     $False = False

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r.BooleanN = CAST($False AS BOOLEAN)

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB
DECLARE $TrueN  -- Boolean
SET     $TrueN = True

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r.BooleanN = CAST($TrueN AS BOOLEAN)

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB
DECLARE $FalseN  -- Boolean
SET     $FalseN = False

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r.BooleanN = CAST($FalseN AS BOOLEAN)

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r.BooleanN IS NULL

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB
DECLARE $True  -- Boolean
SET     $True = True

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r."Boolean" <> CAST($True AS BOOLEAN)

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB
DECLARE $False  -- Boolean
SET     $False = False

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r."Boolean" <> CAST($False AS BOOLEAN)

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB
DECLARE $TrueN  -- Boolean
SET     $TrueN = True

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r."Boolean" <> CAST($TrueN AS BOOLEAN)

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB
DECLARE $FalseN  -- Boolean
SET     $FalseN = False

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r."Boolean" <> CAST($FalseN AS BOOLEAN)

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB
DECLARE $True  -- Boolean
SET     $True = True

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r.BooleanN <> CAST($True AS BOOLEAN) OR r.BooleanN IS NULL

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB
DECLARE $False  -- Boolean
SET     $False = False

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r.BooleanN <> CAST($False AS BOOLEAN) OR r.BooleanN IS NULL

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB
DECLARE $TrueN  -- Boolean
SET     $TrueN = True

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r.BooleanN <> CAST($TrueN AS BOOLEAN) OR r.BooleanN IS NULL

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB
DECLARE $FalseN  -- Boolean
SET     $FalseN = False

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r.BooleanN <> CAST($FalseN AS BOOLEAN) OR r.BooleanN IS NULL

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r.BooleanN IS NOT NULL

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	g_1.Id,
	COUNT(CASE
		WHEN g_1."Boolean" THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Boolean" = True THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.BooleanN = True THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Boolean" = False THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.BooleanN = False THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Boolean" = False THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.BooleanN = False OR g_1.BooleanN IS NULL THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Boolean" = True THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.BooleanN = True OR g_1.BooleanN IS NULL THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.Int32 = 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.Int32N = 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Decimal" = 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.DecimalN = 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Double" = 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.DoubleN = 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.Int32 <> 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.Int32N <> 0 OR g_1.Int32N IS NULL THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Decimal" <> 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.DecimalN <> 0 OR g_1.DecimalN IS NULL THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Double" <> 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.DoubleN <> 0 OR g_1.DoubleN IS NULL THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.Int32 > 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.Int32N > 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Decimal" > 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.DecimalN > 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Double" > 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.DoubleN > 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.Int32 < 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.Int32N < 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Decimal" < 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.DecimalN < 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Double" < 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.DoubleN < 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.Int32 >= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.Int32N >= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Decimal" >= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.DecimalN >= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Double" >= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.DoubleN >= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.Int32 <= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.Int32N <= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Decimal" <= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.DecimalN <= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Double" <= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.DoubleN <= 0 THEN 1
		ELSE NULL
	END)
FROM
	BooleanTable g_1
GROUP BY
	g_1.Id

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	r.Id,
	r.Int32 = 0,
	CASE
		WHEN r.Int32N = 0 THEN True
		ELSE False
	END,
	r."Decimal" = 0,
	CASE
		WHEN r.DecimalN = 0 THEN True
		ELSE False
	END,
	r."Double" = 0,
	CASE
		WHEN r.DoubleN = 0 THEN True
		ELSE False
	END,
	Coalesce(r.Int32N, r.Int32) = 0,
	Coalesce(r.DecimalN, r."Decimal") = 0,
	Coalesce(r.DoubleN, r."Double") = 0,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.Int32N
			ELSE r.Int32
		END = 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.Int32N
			ELSE r.Int32
		END = 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.DecimalN
			ELSE r."Decimal"
		END = 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.DecimalN
			ELSE r."Decimal"
		END = 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.DoubleN
			ELSE r."Double"
		END = 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.DoubleN
			ELSE r."Double"
		END = 0
			THEN True
		ELSE False
	END,
	r.Int32 > 0,
	CASE
		WHEN r.Int32N > 0 THEN True
		ELSE False
	END,
	r."Decimal" > 0,
	CASE
		WHEN r.DecimalN > 0 THEN True
		ELSE False
	END,
	r."Double" > 0,
	CASE
		WHEN r.DoubleN > 0 THEN True
		ELSE False
	END,
	Coalesce(r.Int32N, r.Int32) > 0,
	Coalesce(r.DecimalN, r."Decimal") > 0,
	Coalesce(r.DoubleN, r."Double") > 0,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.Int32N
			ELSE r.Int32
		END > 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.Int32N
			ELSE r.Int32
		END > 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.DecimalN
			ELSE r."Decimal"
		END > 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.DecimalN
			ELSE r."Decimal"
		END > 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.DoubleN
			ELSE r."Double"
		END > 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.DoubleN
			ELSE r."Double"
		END > 0
			THEN True
		ELSE False
	END,
	r.Int32 >= 0,
	CASE
		WHEN r.Int32N >= 0 THEN True
		ELSE False
	END,
	r."Decimal" >= 0,
	CASE
		WHEN r.DecimalN >= 0 THEN True
		ELSE False
	END,
	r."Double" >= 0,
	CASE
		WHEN r.DoubleN >= 0 THEN True
		ELSE False
	END,
	Coalesce(r.Int32N, r.Int32) >= 0,
	Coalesce(r.DecimalN, r."Decimal") >= 0,
	Coalesce(r.DoubleN, r."Double") >= 0,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.Int32N
			ELSE r.Int32
		END >= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.Int32N
			ELSE r.Int32
		END >= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.DecimalN
			ELSE r."Decimal"
		END >= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.DecimalN
			ELSE r."Decimal"
		END >= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.DoubleN
			ELSE r."Double"
		END >= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.DoubleN
			ELSE r."Double"
		END >= 0
			THEN True
		ELSE False
	END,
	r.Int32 < 0,
	CASE
		WHEN r.Int32N < 0 THEN True
		ELSE False
	END,
	r."Decimal" < 0,
	CASE
		WHEN r.DecimalN < 0 THEN True
		ELSE False
	END,
	r."Double" < 0,
	CASE
		WHEN r.DoubleN < 0 THEN True
		ELSE False
	END,
	Coalesce(r.Int32N, r.Int32) < 0,
	Coalesce(r.DecimalN, r."Decimal") < 0,
	Coalesce(r.DoubleN, r."Double") < 0,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.Int32N
			ELSE r.Int32
		END < 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.Int32N
			ELSE r.Int32
		END < 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.DecimalN
			ELSE r."Decimal"
		END < 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.DecimalN
			ELSE r."Decimal"
		END < 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.DoubleN
			ELSE r."Double"
		END < 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.DoubleN
			ELSE r."Double"
		END < 0
			THEN True
		ELSE False
	END,
	r.Int32 <= 0,
	CASE
		WHEN r.Int32N <= 0 THEN True
		ELSE False
	END,
	r."Decimal" <= 0,
	CASE
		WHEN r.DecimalN <= 0 THEN True
		ELSE False
	END,
	r."Double" <= 0,
	CASE
		WHEN r.DoubleN <= 0 THEN True
		ELSE False
	END,
	Coalesce(r.Int32N, r.Int32) <= 0,
	Coalesce(r.DecimalN, r."Decimal") <= 0,
	Coalesce(r.DoubleN, r."Double") <= 0,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.Int32N
			ELSE r.Int32
		END <= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.Int32N
			ELSE r.Int32
		END <= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.DecimalN
			ELSE r."Decimal"
		END <= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.DecimalN
			ELSE r."Decimal"
		END <= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.DoubleN
			ELSE r."Double"
		END <= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.DoubleN
			ELSE r."Double"
		END <= 0
			THEN True
		ELSE False
	END
FROM
	BooleanTable r
UNION ALL
SELECT
	r_1.Id,
	r_1.Int32 = 0,
	CASE
		WHEN r_1.Int32N = 0 THEN True
		ELSE False
	END,
	r_1."Decimal" = 0,
	CASE
		WHEN r_1.DecimalN = 0 THEN True
		ELSE False
	END,
	r_1."Double" = 0,
	CASE
		WHEN r_1.DoubleN = 0 THEN True
		ELSE False
	END,
	Coalesce(r_1.Int32N, r_1.Int32) = 0,
	Coalesce(r_1.DecimalN, r_1."Decimal") = 0,
	Coalesce(r_1.DoubleN, r_1."Double") = 0,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.Int32N
			ELSE r_1.Int32
		END = 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.Int32N
			ELSE r_1.Int32
		END = 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.DecimalN
			ELSE r_1."Decimal"
		END = 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.DecimalN
			ELSE r_1."Decimal"
		END = 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.DoubleN
			ELSE r_1."Double"
		END = 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.DoubleN
			ELSE r_1."Double"
		END = 0
			THEN True
		ELSE False
	END,
	r_1.Int32 > 0,
	CASE
		WHEN r_1.Int32N > 0 THEN True
		ELSE False
	END,
	r_1."Decimal" > 0,
	CASE
		WHEN r_1.DecimalN > 0 THEN True
		ELSE False
	END,
	r_1."Double" > 0,
	CASE
		WHEN r_1.DoubleN > 0 THEN True
		ELSE False
	END,
	Coalesce(r_1.Int32N, r_1.Int32) > 0,
	Coalesce(r_1.DecimalN, r_1."Decimal") > 0,
	Coalesce(r_1.DoubleN, r_1."Double") > 0,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.Int32N
			ELSE r_1.Int32
		END > 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.Int32N
			ELSE r_1.Int32
		END > 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.DecimalN
			ELSE r_1."Decimal"
		END > 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.DecimalN
			ELSE r_1."Decimal"
		END > 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.DoubleN
			ELSE r_1."Double"
		END > 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.DoubleN
			ELSE r_1."Double"
		END > 0
			THEN True
		ELSE False
	END,
	r_1.Int32 >= 0,
	CASE
		WHEN r_1.Int32N >= 0 THEN True
		ELSE False
	END,
	r_1."Decimal" >= 0,
	CASE
		WHEN r_1.DecimalN >= 0 THEN True
		ELSE False
	END,
	r_1."Double" >= 0,
	CASE
		WHEN r_1.DoubleN >= 0 THEN True
		ELSE False
	END,
	Coalesce(r_1.Int32N, r_1.Int32) >= 0,
	Coalesce(r_1.DecimalN, r_1."Decimal") >= 0,
	Coalesce(r_1.DoubleN, r_1."Double") >= 0,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.Int32N
			ELSE r_1.Int32
		END >= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.Int32N
			ELSE r_1.Int32
		END >= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.DecimalN
			ELSE r_1."Decimal"
		END >= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.DecimalN
			ELSE r_1."Decimal"
		END >= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.DoubleN
			ELSE r_1."Double"
		END >= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.DoubleN
			ELSE r_1."Double"
		END >= 0
			THEN True
		ELSE False
	END,
	r_1.Int32 < 0,
	CASE
		WHEN r_1.Int32N < 0 THEN True
		ELSE False
	END,
	r_1."Decimal" < 0,
	CASE
		WHEN r_1.DecimalN < 0 THEN True
		ELSE False
	END,
	r_1."Double" < 0,
	CASE
		WHEN r_1.DoubleN < 0 THEN True
		ELSE False
	END,
	Coalesce(r_1.Int32N, r_1.Int32) < 0,
	Coalesce(r_1.DecimalN, r_1."Decimal") < 0,
	Coalesce(r_1.DoubleN, r_1."Double") < 0,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.Int32N
			ELSE r_1.Int32
		END < 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.Int32N
			ELSE r_1.Int32
		END < 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.DecimalN
			ELSE r_1."Decimal"
		END < 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.DecimalN
			ELSE r_1."Decimal"
		END < 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.DoubleN
			ELSE r_1."Double"
		END < 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.DoubleN
			ELSE r_1."Double"
		END < 0
			THEN True
		ELSE False
	END,
	r_1.Int32 <= 0,
	CASE
		WHEN r_1.Int32N <= 0 THEN True
		ELSE False
	END,
	r_1."Decimal" <= 0,
	CASE
		WHEN r_1.DecimalN <= 0 THEN True
		ELSE False
	END,
	r_1."Double" <= 0,
	CASE
		WHEN r_1.DoubleN <= 0 THEN True
		ELSE False
	END,
	Coalesce(r_1.Int32N, r_1.Int32) <= 0,
	Coalesce(r_1.DecimalN, r_1."Decimal") <= 0,
	Coalesce(r_1.DoubleN, r_1."Double") <= 0,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.Int32N
			ELSE r_1.Int32
		END <= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.Int32N
			ELSE r_1.Int32
		END <= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.DecimalN
			ELSE r_1."Decimal"
		END <= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.DecimalN
			ELSE r_1."Decimal"
		END <= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.DoubleN
			ELSE r_1."Double"
		END <= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.DoubleN
			ELSE r_1."Double"
		END <= 0
			THEN True
		ELSE False
	END
FROM
	BooleanTable r_1

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r."Boolean" = True

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r."Boolean" = False

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r."Boolean" = True

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r."Boolean" = False

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	1 = 0

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r.BooleanN = True

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r.BooleanN = False

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r.BooleanN = True

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r.BooleanN = False

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r.BooleanN IS NULL

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r."Boolean" <> True

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r."Boolean" <> False

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r."Boolean" <> True

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r."Boolean" <> False

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r.BooleanN <> True OR r.BooleanN IS NULL

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r.BooleanN <> False OR r.BooleanN IS NULL

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r.BooleanN <> True OR r.BooleanN IS NULL

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r.BooleanN <> False OR r.BooleanN IS NULL

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	r.Id,
	r."Boolean",
	r.BooleanN,
	r.Int32,
	r.Int32N,
	r."Decimal",
	r.DecimalN,
	r."Double",
	r.DoubleN
FROM
	BooleanTable r
WHERE
	r.BooleanN IS NOT NULL

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	g_1.Id,
	COUNT(CASE
		WHEN g_1."Boolean" THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Boolean" = True THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.BooleanN = True THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Boolean" = False THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.BooleanN = False THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Boolean" = False THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.BooleanN = False OR g_1.BooleanN IS NULL THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Boolean" = True THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.BooleanN = True OR g_1.BooleanN IS NULL THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.Int32 = 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.Int32N = 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Decimal" = 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.DecimalN = 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Double" = 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.DoubleN = 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.Int32 <> 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.Int32N <> 0 OR g_1.Int32N IS NULL THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Decimal" <> 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.DecimalN <> 0 OR g_1.DecimalN IS NULL THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Double" <> 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.DoubleN <> 0 OR g_1.DoubleN IS NULL THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.Int32 > 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.Int32N > 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Decimal" > 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.DecimalN > 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Double" > 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.DoubleN > 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.Int32 < 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.Int32N < 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Decimal" < 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.DecimalN < 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Double" < 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.DoubleN < 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.Int32 >= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.Int32N >= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Decimal" >= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.DecimalN >= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Double" >= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.DoubleN >= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.Int32 <= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.Int32N <= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Decimal" <= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.DecimalN <= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1."Double" <= 0 THEN 1
		ELSE NULL
	END),
	COUNT(CASE
		WHEN g_1.DoubleN <= 0 THEN 1
		ELSE NULL
	END)
FROM
	BooleanTable g_1
GROUP BY
	g_1.Id

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

-- DuckDB

SELECT
	r.Id,
	r.Int32 = 0,
	CASE
		WHEN r.Int32N = 0 THEN True
		ELSE False
	END,
	r."Decimal" = 0,
	CASE
		WHEN r.DecimalN = 0 THEN True
		ELSE False
	END,
	r."Double" = 0,
	CASE
		WHEN r.DoubleN = 0 THEN True
		ELSE False
	END,
	Coalesce(r.Int32N, r.Int32) = 0,
	Coalesce(r.DecimalN, r."Decimal") = 0,
	Coalesce(r.DoubleN, r."Double") = 0,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.Int32N
			ELSE r.Int32
		END = 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.Int32N
			ELSE r.Int32
		END = 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.DecimalN
			ELSE r."Decimal"
		END = 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.DecimalN
			ELSE r."Decimal"
		END = 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.DoubleN
			ELSE r."Double"
		END = 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.DoubleN
			ELSE r."Double"
		END = 0
			THEN True
		ELSE False
	END,
	r.Int32 > 0,
	CASE
		WHEN r.Int32N > 0 THEN True
		ELSE False
	END,
	r."Decimal" > 0,
	CASE
		WHEN r.DecimalN > 0 THEN True
		ELSE False
	END,
	r."Double" > 0,
	CASE
		WHEN r.DoubleN > 0 THEN True
		ELSE False
	END,
	Coalesce(r.Int32N, r.Int32) > 0,
	Coalesce(r.DecimalN, r."Decimal") > 0,
	Coalesce(r.DoubleN, r."Double") > 0,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.Int32N
			ELSE r.Int32
		END > 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.Int32N
			ELSE r.Int32
		END > 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.DecimalN
			ELSE r."Decimal"
		END > 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.DecimalN
			ELSE r."Decimal"
		END > 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.DoubleN
			ELSE r."Double"
		END > 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.DoubleN
			ELSE r."Double"
		END > 0
			THEN True
		ELSE False
	END,
	r.Int32 >= 0,
	CASE
		WHEN r.Int32N >= 0 THEN True
		ELSE False
	END,
	r."Decimal" >= 0,
	CASE
		WHEN r.DecimalN >= 0 THEN True
		ELSE False
	END,
	r."Double" >= 0,
	CASE
		WHEN r.DoubleN >= 0 THEN True
		ELSE False
	END,
	Coalesce(r.Int32N, r.Int32) >= 0,
	Coalesce(r.DecimalN, r."Decimal") >= 0,
	Coalesce(r.DoubleN, r."Double") >= 0,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.Int32N
			ELSE r.Int32
		END >= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.Int32N
			ELSE r.Int32
		END >= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.DecimalN
			ELSE r."Decimal"
		END >= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.DecimalN
			ELSE r."Decimal"
		END >= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.DoubleN
			ELSE r."Double"
		END >= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.DoubleN
			ELSE r."Double"
		END >= 0
			THEN True
		ELSE False
	END,
	r.Int32 < 0,
	CASE
		WHEN r.Int32N < 0 THEN True
		ELSE False
	END,
	r."Decimal" < 0,
	CASE
		WHEN r.DecimalN < 0 THEN True
		ELSE False
	END,
	r."Double" < 0,
	CASE
		WHEN r.DoubleN < 0 THEN True
		ELSE False
	END,
	Coalesce(r.Int32N, r.Int32) < 0,
	Coalesce(r.DecimalN, r."Decimal") < 0,
	Coalesce(r.DoubleN, r."Double") < 0,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.Int32N
			ELSE r.Int32
		END < 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.Int32N
			ELSE r.Int32
		END < 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.DecimalN
			ELSE r."Decimal"
		END < 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.DecimalN
			ELSE r."Decimal"
		END < 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.DoubleN
			ELSE r."Double"
		END < 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.DoubleN
			ELSE r."Double"
		END < 0
			THEN True
		ELSE False
	END,
	r.Int32 <= 0,
	CASE
		WHEN r.Int32N <= 0 THEN True
		ELSE False
	END,
	r."Decimal" <= 0,
	CASE
		WHEN r.DecimalN <= 0 THEN True
		ELSE False
	END,
	r."Double" <= 0,
	CASE
		WHEN r.DoubleN <= 0 THEN True
		ELSE False
	END,
	Coalesce(r.Int32N, r.Int32) <= 0,
	Coalesce(r.DecimalN, r."Decimal") <= 0,
	Coalesce(r.DoubleN, r."Double") <= 0,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.Int32N
			ELSE r.Int32
		END <= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.Int32N
			ELSE r.Int32
		END <= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.DecimalN
			ELSE r."Decimal"
		END <= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.DecimalN
			ELSE r."Decimal"
		END <= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r."Boolean" THEN r.DoubleN
			ELSE r."Double"
		END <= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r.BooleanN = False THEN r.DoubleN
			ELSE r."Double"
		END <= 0
			THEN True
		ELSE False
	END
FROM
	BooleanTable r
UNION ALL
SELECT
	r_1.Id,
	r_1.Int32 = 0,
	CASE
		WHEN r_1.Int32N = 0 THEN True
		ELSE False
	END,
	r_1."Decimal" = 0,
	CASE
		WHEN r_1.DecimalN = 0 THEN True
		ELSE False
	END,
	r_1."Double" = 0,
	CASE
		WHEN r_1.DoubleN = 0 THEN True
		ELSE False
	END,
	Coalesce(r_1.Int32N, r_1.Int32) = 0,
	Coalesce(r_1.DecimalN, r_1."Decimal") = 0,
	Coalesce(r_1.DoubleN, r_1."Double") = 0,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.Int32N
			ELSE r_1.Int32
		END = 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.Int32N
			ELSE r_1.Int32
		END = 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.DecimalN
			ELSE r_1."Decimal"
		END = 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.DecimalN
			ELSE r_1."Decimal"
		END = 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.DoubleN
			ELSE r_1."Double"
		END = 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.DoubleN
			ELSE r_1."Double"
		END = 0
			THEN True
		ELSE False
	END,
	r_1.Int32 > 0,
	CASE
		WHEN r_1.Int32N > 0 THEN True
		ELSE False
	END,
	r_1."Decimal" > 0,
	CASE
		WHEN r_1.DecimalN > 0 THEN True
		ELSE False
	END,
	r_1."Double" > 0,
	CASE
		WHEN r_1.DoubleN > 0 THEN True
		ELSE False
	END,
	Coalesce(r_1.Int32N, r_1.Int32) > 0,
	Coalesce(r_1.DecimalN, r_1."Decimal") > 0,
	Coalesce(r_1.DoubleN, r_1."Double") > 0,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.Int32N
			ELSE r_1.Int32
		END > 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.Int32N
			ELSE r_1.Int32
		END > 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.DecimalN
			ELSE r_1."Decimal"
		END > 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.DecimalN
			ELSE r_1."Decimal"
		END > 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.DoubleN
			ELSE r_1."Double"
		END > 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.DoubleN
			ELSE r_1."Double"
		END > 0
			THEN True
		ELSE False
	END,
	r_1.Int32 >= 0,
	CASE
		WHEN r_1.Int32N >= 0 THEN True
		ELSE False
	END,
	r_1."Decimal" >= 0,
	CASE
		WHEN r_1.DecimalN >= 0 THEN True
		ELSE False
	END,
	r_1."Double" >= 0,
	CASE
		WHEN r_1.DoubleN >= 0 THEN True
		ELSE False
	END,
	Coalesce(r_1.Int32N, r_1.Int32) >= 0,
	Coalesce(r_1.DecimalN, r_1."Decimal") >= 0,
	Coalesce(r_1.DoubleN, r_1."Double") >= 0,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.Int32N
			ELSE r_1.Int32
		END >= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.Int32N
			ELSE r_1.Int32
		END >= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.DecimalN
			ELSE r_1."Decimal"
		END >= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.DecimalN
			ELSE r_1."Decimal"
		END >= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.DoubleN
			ELSE r_1."Double"
		END >= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.DoubleN
			ELSE r_1."Double"
		END >= 0
			THEN True
		ELSE False
	END,
	r_1.Int32 < 0,
	CASE
		WHEN r_1.Int32N < 0 THEN True
		ELSE False
	END,
	r_1."Decimal" < 0,
	CASE
		WHEN r_1.DecimalN < 0 THEN True
		ELSE False
	END,
	r_1."Double" < 0,
	CASE
		WHEN r_1.DoubleN < 0 THEN True
		ELSE False
	END,
	Coalesce(r_1.Int32N, r_1.Int32) < 0,
	Coalesce(r_1.DecimalN, r_1."Decimal") < 0,
	Coalesce(r_1.DoubleN, r_1."Double") < 0,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.Int32N
			ELSE r_1.Int32
		END < 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.Int32N
			ELSE r_1.Int32
		END < 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.DecimalN
			ELSE r_1."Decimal"
		END < 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.DecimalN
			ELSE r_1."Decimal"
		END < 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.DoubleN
			ELSE r_1."Double"
		END < 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.DoubleN
			ELSE r_1."Double"
		END < 0
			THEN True
		ELSE False
	END,
	r_1.Int32 <= 0,
	CASE
		WHEN r_1.Int32N <= 0 THEN True
		ELSE False
	END,
	r_1."Decimal" <= 0,
	CASE
		WHEN r_1.DecimalN <= 0 THEN True
		ELSE False
	END,
	r_1."Double" <= 0,
	CASE
		WHEN r_1.DoubleN <= 0 THEN True
		ELSE False
	END,
	Coalesce(r_1.Int32N, r_1.Int32) <= 0,
	Coalesce(r_1.DecimalN, r_1."Decimal") <= 0,
	Coalesce(r_1.DoubleN, r_1."Double") <= 0,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.Int32N
			ELSE r_1.Int32
		END <= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.Int32N
			ELSE r_1.Int32
		END <= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.DecimalN
			ELSE r_1."Decimal"
		END <= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.DecimalN
			ELSE r_1."Decimal"
		END <= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1."Boolean" THEN r_1.DoubleN
			ELSE r_1."Double"
		END <= 0
			THEN True
		ELSE False
	END,
	CASE
		WHEN CASE
			WHEN r_1.BooleanN = False THEN r_1.DoubleN
			ELSE r_1."Double"
		END <= 0
			THEN True
		ELSE False
	END
FROM
	BooleanTable r_1

-- DuckDB

SELECT
	t1.Id,
	t1."Boolean",
	t1.BooleanN,
	t1.Int32,
	t1.Int32N,
	t1."Decimal",
	t1.DecimalN,
	t1."Double",
	t1.DoubleN
FROM
	BooleanTable t1

