...
SELECT
	...
	COUNT(p.ParentID) OVER (PARTITION BY p.Value1 ORDER BY p.Value1 DESC RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW),
	COUNT(*) OVER (),
	CAST(CAST(RANK() OVER (PARTITION BY p.Value1, c_1.ChildID ORDER BY p.Value1, c_1.ChildID) AS BIGINT) AS DOUBLE) + Sqrt(CAST(DENSE_RANK() OVER (PARTITION BY p.Value1, c_1.ChildID ORDER BY p.Value1) AS DOUBLE)) + COUNT(p.ParentID) OVER (PARTITION BY p.Value1 ORDER BY p.Value1 RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) + COUNT(*) OVER ()
FROM
	Parent p
