...
LEFT JOIN (
    ...
    INNER JOIN (
        ...
        WHERE @ef_filter__p1 OR NOT (t."IsDeleted") OR NOT (t."IsDeleted")
    ) AS t0 ON e1."TerritoryID" = t0."TerritoryID"
    WHERE @ef_filter__p1 OR NOT (e1."IsDeleted") OR NOT (e1."IsDeleted")
...
LEFT JOIN (
    ...
    INNER JOIN (
        ...
        FROM "Products" AS p
        WHERE (@ef_filter__p11 OR p."ProductID" > 2) AND (@ef_filter__p11 OR NOT (p."Discontinued")) AND (@ef_filter__p1 OR NOT (p."IsDeleted") OR NOT (p."IsDeleted"))
    ) AS p0 ON o0."ProductID" = p0."ProductID"
    WHERE @ef_filter__p1 OR NOT (o0."IsDeleted") OR NOT (o0."IsDeleted")
) AS s0 ON o."OrderID" = s0."OrderID"
WHERE @ef_filter__p1 OR NOT (o."IsDeleted") OR NOT (o."IsDeleted")
