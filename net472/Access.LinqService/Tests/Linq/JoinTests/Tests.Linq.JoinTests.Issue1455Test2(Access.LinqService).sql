...
BeforeExecute
-- Access AccessOleDb
DECLARE @p1 VarWChar(3) -- String
SET     @p1 = '%C%'
DECLARE @p2 VarWChar(3) -- String
SET     @p2 = '%C%'
DECLARE @p3 VarWChar(3) -- String
SET     @p3 = '%C%'
...
FROM
	(( 
		...
		LEFT JOIN [Trade] [trade1] ON (([al_1].[alert] IS NOT NULL AND [al_1].[alert] = CStr([trade1].[DealId]))))
		LEFT JOIN [Nomin] [nomin1] ON (([al_1].[alert] IS NOT NULL AND [al_1].[alert] = CStr([nomin1].[CargoId])))
WHERE
