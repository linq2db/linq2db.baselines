...
WHERE
	...
	Convert(VARBINARY(8000), SUBSTRING([p].[FirstName], 1, @nameToCheck_2_1)) = Convert(VARBINARY(8000), @nameToCheck_2_2) AND
	[p].[PersonID] = 1
...
FROM
	[Person] [p]
WHERE
	([p].[FirstName] NOT LIKE @nameToCheck_2 ESCAPE '~' OR Convert(VARBINARY(8000), SUBSTRING([p].[FirstName], 1, @nameToCheck_2_1)) <> Convert(VARBINARY(8000), @nameToCheck_2_2)) AND
	[p].[PersonID] = 1
