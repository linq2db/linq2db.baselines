...
FROM
	(
		...
		FROM
			[Orders] [e]
				LEFT JOIN [Employees] [a_Employee] ON [e].[EmployeeID] = [a_Employee].[EmployeeID] AND [e].[EmployeeID] IS NOT NULL AND [a_Employee].[IsDeleted] = 0
		WHERE
			[e].[IsDeleted] = 0
	) [m_1]
...
FROM
	[Orders] [o]
		LEFT JOIN [Employees] [a_Employee] ON [o].[EmployeeID] = [a_Employee].[EmployeeID] AND [o].[EmployeeID] IS NOT NULL AND [a_Employee].[IsDeleted] = 0
WHERE
	[o].[IsDeleted] = 0
