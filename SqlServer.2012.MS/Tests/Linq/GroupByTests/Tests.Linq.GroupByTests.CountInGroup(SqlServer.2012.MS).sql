...
SELECT
	...
	(
		...
		WHERE
			(Convert(Int, [x_1].[DataValue]) % 2) = 0 AND [x_1].[DataValue] IS NOT NULL
	),
	(
		SELECT
			COUNT(*)
	...
	(
		...
		WHERE
			(Convert(Int, [x_3].[DataValue]) % 2) = 0 AND [x_3].[DataValue] IS NOT NULL
	),
	[t1].[COUNT_5],
	(
		SELECT
...
FROM
	(
		SELECT
			[t].[GroupId] as [Key_1],
			COUNT(*) as [COUNT_1],
			COUNT(IIF((Convert(Int, [t].[DataValue]) % 2) = 0 AND [t].[DataValue] IS NOT NULL, 1, NULL)) as [COUNT_2],
			COUNT(*) as [COUNT_3],
			COUNT(DISTINCT [t].[DataValue]) as [COUNT_4],
			COUNT(IIF((Convert(Int, [t].[DataValue]) % 2) = 0 AND [t].[DataValue] IS NOT NULL, 1, NULL)) as [COUNT_5]
		FROM
			[AggregationData] [t]
		WHERE
