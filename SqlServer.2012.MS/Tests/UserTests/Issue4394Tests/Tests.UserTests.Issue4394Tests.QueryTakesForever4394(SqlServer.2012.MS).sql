...
FROM
	[ChannelDTO] [x_16]
		...
		CROSS APPLY (
			...
			WHERE
				[x_2].[Status] < 0 AND [x_2].[ResourceID] = [b2].[ResourceID]
		) [t2]
		...
		CROSS APPLY (
			...
			WHERE
				[x_4].[Status] < 0 AND [x_4].[ResourceID] = [b3].[ResourceID]
		) [t3]
		...
		CROSS APPLY (
			...
			WHERE
				[x_6].[Status] < 0 AND [x_6].[ResourceID] = [b4].[ResourceID]
		) [t4]
		...
		CROSS APPLY (
			...
			WHERE
				[x_8].[Status] < 0 AND [x_8].[ResourceID] = [b5].[ResourceID]
		) [t5]
		...
		CROSS APPLY (
			...
			WHERE
				[x_10].[Status] < 0 AND [x_10].[ResourceID] = [b6].[ResourceID]
		) [t6]
		...
		CROSS APPLY (
			...
			WHERE
				[x_12].[Status] < 0 AND [x_12].[ResourceID] = [b7].[ResourceID]
		) [t7]
		...
		CROSS APPLY (
			...
			WHERE
				[x_14].[Status] < 0 AND [x_14].[ResourceID] = [b8].[ResourceID]
		) [t8]
...
SELECT
	...
	IIF([a3].[Id] IS NOT NULL AND ([a3].[Id] <> @Empty_2 OR [a3].[Id] IS NULL), 1, 0),
	[a3].[Id],
	[c3].[Id],
	[c3].[ResourceLabel],
	[c3].[Status],
	...
	IIF([a4].[Id] IS NOT NULL AND ([a4].[Id] <> @Empty_3 OR [a4].[Id] IS NULL), 1, 0),
	[a4].[Id],
	[c4].[Id],
	[c4].[ResourceLabel],
	[c4].[Status],
	...
	IIF([a5].[Id] IS NOT NULL AND ([a5].[Id] <> @Empty_4 OR [a5].[Id] IS NULL), 1, 0),
	[a5].[Id],
	[c5].[Id],
	[c5].[ResourceLabel],
	[c5].[Status],
	...
	IIF([a6].[Id] IS NOT NULL AND ([a6].[Id] <> @Empty_5 OR [a6].[Id] IS NULL), 1, 0),
	[a6].[Id],
	[c6].[Id],
	[c6].[ResourceLabel],
	[c6].[Status],
	...
	IIF([a7].[Id] IS NOT NULL AND ([a7].[Id] <> @Empty_6 OR [a7].[Id] IS NULL), 1, 0),
	[a7].[Id],
	[c7].[Id],
	[c7].[ResourceLabel],
	[c7].[Status],
	...
	IIF([a8].[Id] IS NOT NULL AND ([a8].[Id] <> @Empty_7 OR [a8].[Id] IS NULL), 1, 0),
	[a8].[Id],
	[c8].[Id],
	[c8].[ResourceLabel],
	[c8].[Status],
